...
`v0.0.3.91` 是当前稳定版本，已完成 immutable tag、PyPI publish 和全局安装。`.91` 保持 `.90` 的 owner-protected continuation transport，并将已提供 owner-context handle 的公共失败合同统一为 binding-first：文件系统边界、schema、workspace/task/runner 绑定以及 lease identity / credential possession 不匹配均返回 `owner_context_binding_mismatch`，generation fencing 状态错误继续保持独立合同；reusable credential 仍禁止进入公共 argv、JSON、prompt、usage log、generic diagnostics、child argv 或普通 durable state。`v0.0.3.90` 是不可移动的失败 release tag，只保留历史且不得重用或移动；`v0.0.3.89` 是历史 Agent-first Project Observer Beta。Windows 正式安装态应通过 `scripts/install_acf.ps1` 或 `scripts/update_acf.ps1` canonicalize 为 uv tool bin 下的 `acf.cmd`，同目录不保留 `acf.exe`。
