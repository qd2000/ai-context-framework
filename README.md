...
`v0.0.3.91` 是当前 **binding-first owner-context contract release candidate**；本 worktree 已完成源码、文档与 Windows 完整回归，Linux CI、package smoke、PyPI publish 和 global install 仍需通过后续门禁。本地 release preparation 不等于外部发布。`.91` 保持 `.90` 的 owner-protected continuation transport：校验 handle 的文件系统边界、schema、workspace/task/runner 绑定以及 lease identity / credential possession；这些已提供 handle 的失败统一返回 `owner_context_binding_mismatch`，generation fencing 状态错误保持独立合同；reusable credential 仍禁止进入公共 argv、JSON、prompt、usage log、generic diagnostics、child argv 或普通 durable state。`v0.0.3.90` 是不可移动的失败 release tag，不得重用或移动；`v0.0.3.89` 仍是当前已完成 immutable tag / publish / global install 的 **Agent-first Project Observer Beta 稳定安装态**；canonical Windows `acf` 仍为 `.89`，直到 `.91` 完成外部发布与安装验证。
