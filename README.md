...
- `continuation init|configure|list|migrate|doctor|coordination status|coordination attempt|coordination challenge|claim|assert-owner|heartbeat|progress|effect prepare|effect update|effect list|workspace status|workspace intent|workspace adopt|workspace refresh|reconcile|recover|renew|checkpoint|release|pause|resume|prompt|issue`：为外部 AI scheduler 或人工长任务提供有界续跑控制。`init --profile standard|long-running` 可选择时序配置；已有 task 使用 `continuation configure --profile long-running` 原位更新，无需 `init --force`。long-running profile 为 scheduler 60 分钟、lease TTL 180 分钟、renew 45 分钟、heartbeat 建议 10 分钟、stale threshold 25 分钟；heartbeat 只证明 liveness，不延长 TTL，scheduler 周期只是唤醒频率，不是 Gate 时长。新 runner 先用 `coordination attempt` 登记 bounded attempt；已有 owner 时自动 open/join generation-bound challenge，但 contender 不因此获得写权限。普通可定位项目的 ACF 命令只会在 stderr opportunistic 提示当前 owner generation 的 pending challenge，不构成 ACK；只有通过 `lease_id + generation + fence_token` 的 owner-protected continuation 操作才是 authenticated response，正常 release 则解析为 `owner_released`。challenge timeout 仅产生 `ownership_forfeiture_candidate`，仍必须通过 workspace/HEAD/effect/identity reconcile 后才能 generation+1 recover。旧 owner 已结束或存在 matching ownership-forfeiture evidence 时，`reconcile` 可用 `--effect-key`、`--effect-terminal-status`、`--effect-external-id` 与 `--effect-evidence-ref` 对**已经存在且具有 durable external id** 的 unresolved effect 记录 receipt-bound terminal observation；它不调用外部 submit/collect/cancel，也不在 reconcile 阶段改 effect journal，只有 receipt 仍与 observed effect digest 完全一致时才在 `recover` 中落盘 terminal 状态，unknown、identity mismatch 或 observation drift 继续 fail-closed。新 claim/recover 返回递增 `generation` 与本轮专属 owner `fence_token`，持久态只保存 token hash；旧 generation 的 owner-protected 操作会被拒绝。workspace manifest v3 只保存 bounded Git path/status/content digest ownership metadata 与可选 legacy adoption receipt：初始化时已有修改是受保护 `baseline_external`，显式 `workspace intent` 下产生或继承的修改是 `task_owned`，其余非重叠外部修改保持 external；worktree-level `clean/dirty` 不参与 owner liveness、claim、release、reconcile 或 recover 决策。缺少 manifest 且存在 reviewed legacy WIP 时，`doctor` 会返回 `workspace_provenance_missing` 与 `workspace adopt` next action；`workspace adopt` 只能在无 active owner 时一次性使用，要求把当前全部 changed paths 明确分类为 `--task-owned` 或 `--baseline-external` 并提供 evidence refs，记录 status/content digest/prior generation，且 task-owned 路径继续受 bound Workstream write_scope 约束；HEAD 漂移、路径遗漏/重叠或来源不确定继续 fail-closed。adoption 只创建 workspace provenance，不重置 state/round/effect/coordination 历史，也不要求先制造 Git checkpoint。正常 release 可以携带未提交 `task_owned` WIP 并转为 `ready`，下一 generation 在验证 ownerless handoff 期间 task-owned digest 未漂移后继承原 write intent/WIP；因此 Scheduled round 不要求 Git commit，commit 只在自然语义 checkpoint 创建。同路径/父子路径、task-owned ownerless drift、HEAD/effect/identity 无法解释仍 fail-closed。`continuation prompt` 是唯一 generic Scheduled Task protocol 权威；项目 wrapper 只保留固定身份和项目特有约束，不复制 contention/recovery 状态机。generated prompt 采用 goal-directed continuous execution：bounded 只限制 ownership、写入、副作用和恢复风险，不限制一次 activation 可完成的工作量；当前 stage/next_action 只是恢复入口，Agent 自主决定工作范围；子任务、测试、commit、checkpoint 或 Gate 完成都不是停止理由。任务级硬停止只允许总体目标完成、用户明确暂停、需要新的人工授权/凭据/不可替代决策，或项目访问工具（如 DevSpace）经合理重连仍不可用。状态保存在用户级 `~/.acf`，不会因控制元数据更新污染项目 Git。详细设计见 [Continuation Control](docs/ai/reference/Continuation_Control_Design.md)。

- `observer status|snapshot|interpret|glossary-set|glossary|history`：提供项目级 Global Observer。一个项目只维护一个 Observer runtime namespace，同项目 primary checkout 与 ACF 注册 worktree 统一进入观察域；核心原则是 **read broad / write narrow / control none**。Observer 可以广泛读取 Git、Workstream、continuation、计划和 evidence，但只把结构化观测、历史、语义解释和 `dashboard.html` 写入用户级 `~/.acf/projects/<project-id>/observer/`，不会 claim/challenge/recover continuation，也不会修改 Writer 项目文件。`snapshot --dry-run` 只验证，`snapshot` 原子更新 current/history/self-health 并生成可直接 `file://` 打开的自包含静态 Dashboard；Dashboard 不启动 HTTP/daemon，不依赖外部资源。meaningful history 默认永久保留，只做无损按月 rotation/index；Execution / Progress / Health 分开，缺少明确分母时不生成假进度百分比。`interpret` 由 AI 提供中文人类语义，但必须绑定当前 `source_fingerprint`，CLI 负责 source-drift fail-closed、版本历史、canonical identity、confidence 和 provenance；底层事实变化后旧解释自动标记 stale，不继续冒充当前事实。`glossary-set` / `glossary` 维护项目级解释词典，`history` 可读取 live + rotated timeline/observation/alert/run/interpretation。结构化状态与 HTML 共享 credential-like 值过滤；颜色必须和文字/符号一起表达语义，页面使用克制字号。Observer Core 不硬编码 MCP、电脑、盘符或项目实例；具体项目访问工具只属于外部 scheduler wrapper/adapter，而不是产品核心。

`continuation prompt --json` 同时返回稳定的 `identity`、`current`、`execution_policy`、`project_context` 与 `scheduler_wrapper_contract`。`execution_policy` 明确 goal-directed continuous 模式、Agent 自主工作范围、checkpoint/commit/Gate 非停止语义和四类任务级硬停止条件；`project_context` 暴露 continuation state 中的 `plan_refs`、`constraints` 以及外层 wrapper 必须填写的项目专用约束槽。标准薄 wrapper 只固定 worktree/branch/task/workstream identity，每次唤醒重新执行 `acf continuation prompt` 并遵循当轮返回的 generic protocol，再叠加 tooling、Runtime、resource/VM/node、permission、security、scientific、validation 和 issue-reporting 约束；不得复制一份 contention/recovery/workspace/effect 状态机。任何可能跨当前 owner/tool-call 生命周期继续写项目文件或产生非幂等副作用的 long-lived local subprocess、DevSpace session、Runtime job 或外部 job，在启动前都必须建立 deterministic durable effect/job identity，并在权威终态前保持 unresolved；无法持久识别时不得跨 owner 生命周期运行，formal recovery 继续 fail-closed。纯只读长阻塞调用不需要 writer effect，但返回后、下一次项目写入或非幂等动作前必须重新 `assert-owner`。
...
普通 `baseline_external` 默认仍不可 stage/commit。只有当前 authenticated runner 刚通过 `acf workstream scope-add|merge-request|ready|merge-start|done` 为绑定 Workstream 生成 `docs/ai/active/Workstreams.md` 与 `docs/ai/active/workstreams/` 下当前绑定 Workstream 的 detail 文件时，允许在 lifecycle/merge 边界审阅 exact diff 与 durable ACF command evidence 后，将**仅这两个确定性 control-plane 文件**形成独立 checkpoint。它不扩展普通 Task authority write_scope，也不允许把第三个 baseline/external path 带入提交。
