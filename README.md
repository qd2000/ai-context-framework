...
`v0.0.3.22` 是当前推荐的真实项目接入/升级版本，继承 v0.0.3.21 的 `acf review stale` 稳定 JSON 输出契约，并精炼 clean 状态和按 kind 生成的机械 `next_actions`。普通 usage log 仍只记录元数据，显式 `acf log feedback` 可记录人工反馈正文。`upgrade` 仍应 dry-run first，Workstream 仍保持显式启用。
...
acf version set v0.0.3.22 --dry-run --json
acf status --json
