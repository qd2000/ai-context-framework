...
- `continuation init|configure|doctor|claim|assert-owner|heartbeat|progress|effect prepare|effect update|effect list|workspace status|workspace intent|workspace refresh|reconcile|recover|renew|checkpoint|release|pause|resume|prompt|issue`：为外部 AI scheduler 或人工长任务提供有界续跑控制。`init --profile standard|long-running` 可选择时序配置；已有 task 使用 `continuation configure --profile long-running` 原位更新，无需 `init --force`。long-running profile 为 scheduler 60 分钟、lease TTL 180 分钟、renew 45 分钟、heartbeat 建议 10 分钟、stale threshold 25 分钟；heartbeat 只证明 liveness，不延长 TTL，scheduler 周期只是唤醒频率，不是 Gate 时长。新 claim/recover 返回递增 `generation` 与本轮专属 owner `fence_token`，持久态只保存 token hash；旧 generation 的 owner-protected 操作会被拒绝。workspace manifest v2 只保存 bounded Git path/status/content digest ownership metadata：初始化时已有修改是受保护 `baseline_external`，显式 `workspace intent` 下产生或继承的修改是 `task_owned`，其余非重叠外部修改保持 external；worktree-level `clean/dirty` 不参与 owner liveness、claim、release、reconcile 或 recover 决策。正常 release 可以携带未提交 `task_owned` WIP 并转为 `ready`，下一 generation 在验证 ownerless handoff 期间 task-owned digest 未漂移后继承原 write intent/WIP；因此 Scheduled round 不要求 Git commit，commit 只在自然语义 checkpoint 创建。缺少 manifest 且存在未分类路径时报 `workspace_provenance_missing`，同路径/父子路径、task-owned ownerless drift、HEAD/effect/identity 无法解释才 fail-closed。`doctor` 使用独立 `stale_after_minutes` 判断 fresh/stale；`reconcile/recover` 对可证明 interrupted task WIP 做 generation+1 fencing 并保留 external dirty，不 stash/reset/clean、不自动提交。`continuation prompt` 是 Scheduled Task 的 generic protocol 权威；状态保存在用户级 `~/.acf`，不会因控制元数据更新污染项目 Git。详细设计见 [Continuation Control](docs/ai/reference/Continuation_Control_Design.md)。
