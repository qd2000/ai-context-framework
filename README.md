...
`v0.0.3.90` 是当前 **Continuation sensitive-interface hardening release candidate**；本 worktree 已完成源码、文档、完整回归以及 wheel/sdist package smoke 验证。本地 release preparation 不等于 PyPI publish 或 global install。`.90` 将 owner-protected continuation transport 收敛为绑定 workspace/task/lease/generation/runner 的本地 `owner_context.handle` capability，并禁止 reusable credential 进入公共 argv、JSON、prompt、usage log、generic diagnostics、child argv 或普通 durable state；pre-.90 owner 仅可通过显式本地迁移命令完成 capability 迁移。`v0.0.3.89` 仍是当前已完成 immutable tag / publish / global install 的 **Agent-first Project Observer Beta 稳定安装态**；canonical Windows `acf` 仍为 `.89`，直到 `.90` 完成外部发布与安装验证。
