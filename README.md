...
`v0.0.3.97` 已完成 immutable tag、PyPI 发布（release workflow #39，attempt 2 / success）与全局安装闭合，但它不是许可完整的版本：其 wheel/sdist 均不含 LICENSE、METADATA 也没有任何 License 字段，因为该 tag 早于 LICENSE 落地提交。`.97` 补齐缺陷与欠账：发布 smoke 脚本的运行态隔离、分钟级时间生成命令 `acf clock now`、六域 Gap Matrix 过期行校准。`v0.0.3.98` 是许可完整的发布版本（tag `1b115ed`）：它新增 PEP 639 `license = "MIT"` 与 `license-files = ["LICENSE"]`、把 setuptools 构建下限提到 `>=77.0.3`，并新增 wheel/sdist 的 LICENSE 与 METADATA 确定性回归。`v0.0.3.101` 是当前版本：它让 `archive current-task|task-plan` 写入 `archive/Archive_Index.md` 的「原路径」列与 `acf archive sync` 的渲染同源（此前写死 `无。` 导致每次归档立即产生 drift），并让 `acf doctor` 的两条 archive 告警只列真正不一致的条目；生成块之外的旧行仍保留 `无。` 兜底语义。`v0.0.3.100` 与 `v0.0.3.99` 内容完全一致：它为 ownerless graceful handoff 后仍 dirty 但值得保留的 task-owned WIP 提供 `continuation workspace review-handoff` 审查包与 `claim --handoff-review-file` 原子接管，让同一个 Agent 不必先 commit / stash / revert / 删除现场即可继续处理；`.99` 的 publish 前必需门禁在 Windows + Python 3.12 作业上失败、PyPI 从未产生 `0.0.3.99`，因此按不可变 tag 约定保留为历史失败 release tag，由 `.100` 重新发布。`.96` 是 runtime 全局注入解耦（内部重构，对外 CLI 契约零变化，以命令树形状快照为证）；`.95` 修复 Windows CI 发布门禁的三处测试夹具缺陷并重新闭合发布链；`.94` 起 `acf worktree retire` 提供 curated handoff 退役窄路径。`v0.0.3.93` 与 `v0.0.3.94` 保留为历史失败 release tag，不可改写。Windows 正式安装态应通过 `scripts/install_acf.ps1` 或 `scripts/update_acf.ps1` canonicalize 为 uv tool bin 下的 `acf.cmd`，同目录不保留 `acf.exe`。

- 版本变化：[CHANGELOG.md](CHANGELOG.md)
- Workstream/Worktree 教程：[docs/Worktree_Lifecycle.md](docs/Worktree_Lifecycle.md)
- 详细 CLI 参数：`acf --help`、`acf workstream reserve --help`、`acf worktree --help`、`acf continuation --help`

ACF 的默认 AI 入口采用 global-first progressive disclosure：无论项目有一个还是多个 Active / Blocked / ReadyToMerge / Merging Workstream，只要没有明确选择，agent 都只读取全局上下文和 Workstreams 摘要，不读取任何 WS detail、read_scope、reference、output 或专属 worklog；该选择状态在 `acf status --json` 的 `workstream_state` 中报告为 `GlobalOnly`。显式选择后，`acf status|next --workstream WSNNN` 仍只返回 pointer-only 入口；随后调用 `acf workstream context WSNNN` 才披露专属上下文和边界。`reference/` 是项目内中间材料层，Knowledge 是有来源、证据和适用边界的高可信精炼层，均按需读取。
...
- `init` / `simplify --force` 在执行 destructive replacement 前会保护当前 `ACF_HOME`（默认 `~/.acf`）：若既有 target 等于、位于或包含该 runtime tree，会以 `acf_home_target_protected` fail-closed，避免删除 continuation、closeout authorization 与 usage-log state。普通非破坏性 target 行为保持原语义；destructive dogfood/测试应显式使用隔离的临时 `ACF_HOME`。
...
- `continuation workspace review-handoff` 与 `continuation claim --handoff-review-file` 是漂移现场仍 dirty 但值得保留时的"Agent 自行审查 + 原子接管"路径：Agent 不必先 commit / stash / revert / 删除现场，先用只读的 `review-handoff` 生成按当前 HEAD 与逐路径 status/digest 绑定的审查包和 `--draft-file` 决策草稿（四种决策：`inherit_for_triage` / `preserve_external` / `semantic_clean` / `block`，每条 drift path 恰好一次），再用同一个 runner 的 `claim --handoff-review-file` 在 state lock 内一次性完成机械验证、应用决策、写 `last_handoff_takeover.json` 审计 receipt、推进 generation 与创建 lease。审查后任一相关文件字节/HEAD/status 变化、runner 不一致、scope 冲突、决策未精确解释全部 conflict、HEAD 漂移未显式接受或 unresolved effect 存在，都以稳定错误码 fail-closed，不建 lease、不改工作区。接管成功只表示"已确认值得保留并继续处理"，不表示内容正确或已测试。
- `continuation coordination wait|await` 是 stale-owner challenge 的确定性 blocking waiter，不是 takeover 命令。它只读取一个已持久化 challenge 的终态和 `deadline_at`，本地轮询间隔只影响观察延迟，不改变 challenge 的语义等待窗口；返回 `owner_active` 时让行、`owner_released` 时重新 doctor/正常 claim、`timeout` 时刷新全部 recovery evidence 后 formal reconcile/recover。该命令始终返回 `ownership_granted=false`；pending challenge 不能被当作当前 activation 的结束理由，也不要求模型在无工具活动下“空等”。
...
Observer 产品能力已在 `v0.0.3.92` 从 ACF 核心退役。`acf observer` 只保留一个无副作用的退役入口：它不导入旧实现、不读取或写入 Observer runtime、不创建目录，并稳定返回 `observer_retired` 及 `acf status` / `acf workstream dashboard` / `acf continuation doctor` 替代提示。原 Observer runtime、Dashboard、Target Registry、semantic-review / Map Review、presentation lifecycle、Agent-owned 页面与 narrative 能力均不再发布；`acf continuation prompt --json` 也不再携带 Observer 合同子树。旧机器上的用户级 Observer 数据不会被升级流程自动删除；历史发布记录（包括 `.89` Observer Beta）和 `docs/ai/archive/` 保留为历史事实。
...
- `log enable|disable|status|tail|summarize|projects|feedback|prune|gc|issues|issue`：管理本地使用状态日志，默认开启以便开发调试收集反馈，可用 `log disable` 按项目关闭；`log projects --scan-root <path> --json` 可只读盘点全局日志中的项目并匹配磁盘上的 context root；`log gc` 默认干跑报告孤儿日志 namespace，只有 `--apply` 才清理并写出 receipt；`log issues` 支持分页与 `--summary-only`，`log issue list|show|resolve|supersede|reject|reopen` 维护独立于 continuation 的跨项目 issue 生命周期；普通 usage event 不记录正文，显式 `log feedback --text/--input` 才记录人工反馈正文。
- `version show|set`：查看或一键更新 CLI、包配置和本地元数据版本号。
- `clock now`：输出分钟级本地时间 `YYYY-MM-DD HH:MM`，供上下文里的「上次更新 / 上次修改」等时间字段引用；不依赖上下文目录，也不写文件；支持 `--json`。
...
Copyright (c) 2026 qd2000
```

MIT 许可覆盖本仓库中的代码、`template/` 模板与文档。
