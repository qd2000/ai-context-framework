...
`v0.0.3.89` 是当前 **Agent-first Project Observer Beta 发布候选**；在它完成 immutable tag / publish / global install 前，`v0.0.3.88` 仍是全局稳定安装态。`.89` 保留 Writer continuation/effect/fencing/Git safety 与 Production anti-masking，同时把 Production Observer 默认合同切换为 Agent-first：ACF 只是可选导航、数据和兼容工具，Target Registry、semantic-review、Map Review、`primary_visualization` 与 fixed renderer 均不再是页面生成前置；Observer Agent 可以从当前授权的一手资料自主理解项目并维护 Observer-owned HTML/CSS/SVG/JS。旧 derived-presentation/runtime 能力继续作为显式 legacy compatibility helper，且不得覆盖 Agent-owned 稳定入口。Beta/global install 后，现有 ACF/FCC/AStockT_AI 三个 Production Observer Scheduled Task 的平台提示迁移仍由用户按 WS012 提供的 copy-paste-ready 包手工完成。
