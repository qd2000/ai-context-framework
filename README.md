...
`v0.0.3.55` 是当前推荐的真实项目正式使用稳定版本。该版本在 `v0.0.3.54` 的 Workstream 编号预约与 Git worktree 全生命周期基础上，修正默认上下文路由：新 AI 会话未显式选择 Workstream 时返回 `GlobalOnly`，只暴露全局上下文文件指针和 Workstream 摘要；`attention` 只用于 dashboard 管理，不再自动选择任务。只有显式 `--workstream`、Active `Current_Task` 的唯一绑定或经过 registry/path/branch/common-dir 校验的专属 worktree 才返回单一 Workstream 指针，真正专属内容仍需再调用 `acf workstream context WSNNN`。冲突和无效选择 fail-closed，并保持全局上下文。原 `workstream add/reserve` 与全部 worktree 生命周期接口保持兼容。
...
- `worktree create|attach|verify|list|audit|sync|merge-plan|merge|close|resume`：供 AI 按任务需要调用的可选 Git 生命周期能力。创建 Workstream 不会隐式创建 worktree；`create` 同时支持正式 WS 与 `bugfix/docs/experiment/investigation/maintenance/refactor/release` 非 WS 任务；所有写操作默认只输出计划，显式 `--apply` 后才创建分支、worktree、merge 或关闭。实现使用 Git common-dir 下的 journal、registry 和锁，拒绝 detached、错误仓库绑定、目录覆盖、dirty merge、冲突、force、stash、reset、clean、rebase 和自动 push。完整说明见 [docs/Worktree_Lifecycle.md](docs/Worktree_Lifecycle.md)。
