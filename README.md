...
`v0.0.3.87` 是当前发布候选版本。它把 WS012 已冻结的 continuation physical-execution liveness 修复与 Scheduled Writer graceful handoff / overconstraint reduction 合并进入同一正常稳定版：`acf continuation execution run` 用 generation-bound deterministic effect、精确 `PID + process-start identity` 与自动 heartbeat/renew 证明同一长时间物理进程仍真实存活；authenticated `continuation release --handoff` 则允许 genuine safe session end 释放 owner/lease、结束当前 round，同时保持 long-lived mission `running`，下一 activation 仅在明确 durable handoff marker 存在时恢复 claim。generated execution policy 同步把 wall-clock 降为诊断信号，并明确 active lease、历史异常 generation、task-owned dirty、checkpoint/test/commit/Gate 或单 blocked lane 都不是独立 stop signal。正式稳定身份仍以 immutable tag/publish/global install 与 installed-state physical-liveness + graceful-handoff dogfood 完成为准；Project Observer 继续保持 **read broad / write narrow / control none**，Maintenance Writer 不用例行 Observer 刷新掩盖 Production activation failure。
...
Observer V2 P1.5 在此兼容面之上增加统一的 `automation_prompt_execution_contract`。Writer Scheduled Task 的静态 wrapper 继续负责 sufficient high-salience bootstrap 和命名的 project/task extension slot，动态 owner/generation/directive/current-plan/checkpoint/release/exit 仍只由每轮 `acf continuation prompt + execution_policy` 负责；wrapper 不固化这些 volatile 状态。Writer 的 Observer-useful execution evidence 明确包含 semantic stage / milestone / major outcome / proven / excluded / blocker / **problem** / **plan impact** / route impact / next logic，heartbeat/generation 本身不能冒充语义进展。Production Observer wrapper 则固定 `read broad / write narrow / control none`、existing-checkout、显式 Target Registry、anti-masking、truthful run history、Map Review Gate 与 human-first-but-detailed 展示，并且不复制 Writer ownership state machine。每次 semantic refresh 都必须显式记录 `unchanged / patch / rebuild / presentation_change / project_overview_enable / project_overview_disable` 决策、reason/evidence/fingerprint；fingerprint 只用于 triage，不能替代语义复核。interactive presentation maintenance 只允许三种 lifecycle：即时 presentation-only `transient_patch`、下一次 Production Observer 消费的 `one_shot_semantic_review`、以及可 supersede/withdraw 的 `durable_rule`。记录任何交互请求前，Agent 必须先理解当前 derived semantic/presentation state，并把用户意图整理为 `reviewed_intent / scope / rationale / evidence_refs`；ACF 只负责确定性记录、校验和 optimistic concurrency，不自动裁决自然语言语义。即时 patch 只能走 user-level narrow derived-presentation surface，并要求 deterministic re-render；禁止直接改 `dashboard.html`。所有 semantic-risk 请求必须进入 Map Review + authority reread；presentation revision/fingerprint 冲突时 fail-closed reread/re-evaluate。interactive maintenance 继续遵守 `read broad / write narrow / control none` 与 foreign-project non-interference，one-shot 成功消费后退出 active context且不得复活，durable rule 才能持续并支持 supersede/withdraw。`acf observer status --json` 与 `acf continuation prompt --json` 都暴露同一静态 P1.5 family contract，供 scheduler wrapper、文档和 regression tests 做一致性检查。

Observer V2 P2 把其中的**语义生命周期**落实为 user-level runtime，而不是把自然语言需求直接写进项目 authority。`acf observer presentation-status` 只读返回每个显式 target 的独立 source fingerprint、`not_reviewed|current|stale`、active one-shot request 与 durable rule；一个 target 的事实变化不会让其他 target 一起 stale。`review-request-add` 记录下一次 Production Map Review 必须消费的一次性 reviewed intent；成功 `semantic-review-apply` 后该 request 立即退出 active context，审计事件仍保留，后续 render 不得复活。`presentation-rule-add` 只用于明确长期有效的规则，支持 `--supersedes`，`presentation-rule-withdraw` 明确撤回；所有写操作要求 exact target revision，冲突 fail-closed。`semantic-review-apply` 还要求 exact target source fingerprint、authority fingerprint、reason/evidence；存在 map-relevant signal 时必须显式 `--authority-reread`。target narrative 保存 goal/route/current-position/why-now/proof/next-logic 与 route nodes/edges；problem model 分离 expectedness、handler、scope relation、blocking impact、plan impact 和 lifecycle，`route_change|major_replan` 不能被 `decision=unchanged` 掩盖，`handler=other_project` 只能记录 transfer linkage，不能越权改 foreign project。P2 不实现即时 `transient_patch`；该 narrow derived-presentation surface 与 deterministic re-render 属于 P3。

Observer V2 P3 将 target 页面重建为中文 human-first、自包含 Dashboard：先展示最终目标、完整路线、当前位置与 why-now，再展示最近证明/排除/改变、当前 problem 与 plan impact、下一步及理由；执行范围、run chain、Alerts 与 Timeline 继续保留为可审计证据层。即时 `transient_patch` 只允许修改 user-level derived presentation，并要求 **exact target revision + exact presentation revision + exact presentation fingerprint** 三重乐观并发校验；它只能针对已经存在的 canonical `current.json` target view 执行，随后对同一 canonical snapshot 做 deterministic re-render，既不创建新的 snapshot/run，也不能用更新但尚未被 Production Observer 采集的事实“跨代刷新” Dashboard。缺少 canonical current、presentation view 已变化、target semantic source 已漂移或请求包含 semantic-risk signal 时均 fail-closed；semantic-risk 必须升级为正式 Map Review + authority reread。新的 semantic review 会自动使仍 active 的 transient patch 过期，clear 后也不会复活；one-shot、durable rule 与 transient patch 因而形成相互独立但可组合验收的三种 lifecycle。禁止直接编辑 `dashboard.html`。
