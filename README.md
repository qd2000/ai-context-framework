...
- `continuation init|doctor|claim|assert-owner|heartbeat|progress|effect prepare|effect update|effect list|renew|checkpoint|release|pause|resume|prompt|issue`：为外部 AI scheduler 或人工长任务提供有界续跑控制。新 claim 返回递增 `generation` 与本轮专属的 owner `fence_token`，运行态只持久化 token hash；`assert-owner`、`heartbeat`、`progress`、effect 写操作、`renew`、`checkpoint`、`release` 会拒绝已被新 generation 取代的 owner。`heartbeat` 只更新 runner liveness、不延长 lease TTL；`progress` 只保存 bounded、runtime-neutral 的 phase/milestone/evidence refs；外部/non-idempotent 动作必须先用 `effect prepare` 写入 deterministic logical identity，且只有 `created=true` 才允许首次执行，`created=false` 必须复用/核对而不是重放。effect journal 只保存 compact key/kind/external id/status/milestone/evidence refs，不接受 raw output/transcript；`doctor` 会区分 fresh、stale/orphan candidate、legacy-unknown 和 expired lease，并对“expired running round + durable effect records”返回 `effect_reconciliation_required`，在正式 reconcile/recover 前禁止直接 re-claim。状态保存在用户级 `~/.acf`，不会因为 lease/checkpoint/effect 更新把目标 worktree 变 dirty；可选绑定 `--workstream WSNNN` 时复用 ACF 的 registry/path/branch/common-dir 验证。旧版无 heartbeat/generation/round/effect journal 的任务仍可保守读取并 lazy upgrade。`issue` 让 dogfood agent 把可复用的 ACF/自动化缺口连同证据写入用户级结构化日志，`log issues --all-projects` 可跨 worktree 聚合重复问题。该命令不创建 Scheduled Task、不读取聊天历史、不充当常驻 agent runtime；现有 clean worktree 可以直接初始化，无需重建。详细设计见 [Continuation Control](docs/ai/reference/Continuation_Control_Design.md)。
