...
- `continuation init|doctor|claim|assert-owner|heartbeat|progress|effect prepare|effect update|effect list|workspace status|workspace intent|workspace refresh|reconcile|recover|renew|checkpoint|release|pause|resume|prompt|issue`：为外部 AI scheduler 或人工长任务提供有界续跑控制。新 claim/recover 返回递增 `generation` 与本轮专属 owner `fence_token`，持久态只保存 token hash；旧 generation 的 owner-protected 操作会被拒绝。`progress` 与 effect journal 只保存 bounded runtime-neutral metadata，外部/non-idempotent 动作必须先 `effect prepare`，且只有 `created=true` 才允许首次执行。workspace manifest 只保存 Git 路径/status/digest：`init` 会把已有 dirty 作为受保护 `baseline_external`，active owner 通过 `workspace intent --path ...` 声明具体写入路径，绑定 Workstream 时 intent 必须命中直接 write scope；`workspace refresh` 区分 `runner_owned`、非重叠外部 dirty 与真实 path conflict。非重叠 external dirty 不再因为整个 worktree `clean=false` 就阻塞 active round，也不会被 ACF 自动提交；未 checkpoint 的 runner-owned/conflicting dirty 在 release 时仍 fail-closed。`doctor` 区分 fresh、stale/orphan candidate、legacy-unknown、expired 与 effect reconciliation signal；`reconcile` 默认只读分类，fresh active owner 永远不可 takeover，stale/legacy active 也必须由调用方显式提供 owner-ended evidence，HEAD 前进必须精确接受当前 HEAD，任何 unresolved effect 都保持 blocked。Git dirty 本身不再阻止 stale/orphan recovery：只要 workspace manifest 可验证、generation 与 interrupted owner 一致且无真实 conflict，可证明的 `runner_owned` WIP 可与不重叠的 baseline/external dirty 一起进入 formal recovery。只有 `--record` 生成的 eligible receipt 才能被 `recover` 消费；recover 会再次比对 lease/HEAD/round/effect/workspace ownership observation，workspace digest 漂移会使 receipt stale；成功后 generation+1 fencing，并继承原 write intents 与 runner-owned WIP，继续保护 external dirty，不 stash/reset/clean，也不自动提交 external dirty。错误 `--task-id` 返回 `continuation_task_not_found` 与 `available_tasks`。状态保存在用户级 `~/.acf`，不会因为控制元数据更新污染目标 Git；旧 task 可原位保守升级。该命令不创建 Scheduled Task、不读取聊天历史、不充当常驻 agent runtime。详细设计见 [Continuation Control](docs/ai/reference/Continuation_Control_Design.md)。
