...
`v0.0.3.74` 是当前推荐版本。它继承 `.73` 的 legacy local-effect recovery 和 `.72` 的 sufficient scheduler bootstrap、state-conditioned generated prompt、default execution plan、owner liveness/duplicate-wake、continuation capacity/effect rollover hardening，并正式把 **Project Observer** 纳入稳定产品：同一项目 primary checkout 与注册 worktree 进入同一用户级 Observer namespace，Observer 只读 Git/ACF/计划/evidence，狭窄写入 `~/.acf/.../observer/` 的 current/history/semantic/glossary/static Dashboard，固定为 **read broad / write narrow / control none**，不 claim/challenge/recover Writer continuation。`.74` 同时把 WS011 dogfood 暴露的 reviewed Workstream lifecycle checkpoint exception 适配到当前 progressive prompt，而不是恢复旧的全量 continuation 手册。
