...
`v0.0.3.69` 是当前推荐版本。`acf continuation prompt` 采用 goal-directed continuous execution：scheduler wake 只是持续任务的恢复入口，不定义工作回合、汇报周期或预期停止点；bounded 只约束 ownership、写入、副作用和恢复风险，不限制当前执行 session 的有效工作范围。stage/next_action 只是恢复入口，Agent 自主决定工作范围；子任务、测试、commit、checkpoint 或 Gate 完成都不是停止理由。final response 会结束当前执行 session，因此不能只为汇报进展、经过了一段时间或“感觉该收尾”而触发；平台边界必须有平台、系统或工具的明确终止信号。JSON 同时暴露 `current`、`execution_policy`、`project_context` 和项目专用约束槽，供统一 Scheduled Task 薄 wrapper 填入 DevSpace、Runtime、VM/节点、权限、安全、验证和 issue 记录规则。
