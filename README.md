...
- `continuation init|doctor|claim|assert-owner|heartbeat|progress|effect prepare|effect update|effect list|reconcile|recover|renew|checkpoint|release|pause|resume|prompt|issue`：为外部 AI scheduler 或人工长任务提供有界续跑控制。新 claim/recover 返回递增 `generation` 与本轮专属 owner `fence_token`，持久态只保存 token hash；旧 generation 的 owner-protected 操作会被拒绝。`progress` 与 effect journal 只保存 bounded runtime-neutral metadata，外部/non-idempotent 动作必须先 `effect prepare`，且只有 `created=true` 才允许首次执行。`doctor` 区分 fresh、stale/orphan candidate、legacy-unknown、expired 与 effect reconciliation signal；`reconcile` 默认只读分类，fresh active owner 永远不可 takeover，stale/legacy active 也必须由调用方显式提供 owner-ended evidence，HEAD 前进必须精确接受当前 HEAD，任何 unresolved effect 都保持 blocked。只有 `--record` 生成的 eligible receipt 才能被 `recover` 消费；recover 会再次比对 lease/HEAD/round/effect observation，任何旧 owner heartbeat、HEAD 或 journal 漂移都会使 receipt 失效，然后 generation+1 fencing。clean `release` 在未显式传 `--final-status` 时只把仍为 `running` 的 state 转回 `ready`；已经 checkpoint 为 `waiting_external`、`blocked_human`、`done` 等非 running 状态会被保留。错误 `--task-id` 返回 `continuation_task_not_found` 与 `available_tasks`。dogfood issue 可用 `continuation issue --resolve-fingerprint <fp> --text ...` 追加 resolution event；`log issues --open-only` 只看未解决项，同 fingerprint 后续重新出现会自动 reopen。状态保存在用户级 `~/.acf`，不会因为 lease/checkpoint/effect/recovery receipt 更新把目标 worktree 变 dirty；旧 v0.0.3.61 task 可原位保守 reconcile/recover，不要求 force re-init。该命令不创建 Scheduled Task、不读取聊天历史、不充当常驻 agent runtime。详细设计见 [Continuation Control](docs/ai/reference/Continuation_Control_Design.md)。
