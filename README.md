...
Agent-owned 文本稳定入口还要求在晋升前以 strict UTF-8 回读候选并拒绝 Unicode replacement character（U+FFFD）；但“可 UTF-8 解码且 U+FFFD=0”只是必要条件，不足以证明可见文本未被传输层 mojibake。Agent 还必须用当前 authority/source 派生的预期可见文本 marker（例如项目原生非 ASCII 标签或等价 sentinel）做晋升前回读验证；如果所用命令/PTY 通道的 Unicode 保真未经证明，必须改用 byte-preserving 写入或 ASCII-safe payload transport。任一完整性检查失败都继续走既有 last-good 更新失败策略。该规则只保护文本完整性，不引入统一 renderer 或页面 schema，也不把任何单一语言硬编码进产品合同。
