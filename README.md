...
`v0.0.3.26` 是当前推荐的真实项目接入/升级版本，新增 Workstream 内部阶段注册、聚焦和完成命令，用于让 Workstream 阶段闭环保持机械一致。普通 usage log 仍只记录元数据，显式 `acf log feedback` 可记录人工反馈正文。`upgrade` 仍应 dry-run first，Workstream 仍保持显式启用。
...
acf version set v0.0.3.26 --dry-run --json
acf status --json
