...
`v0.0.3.92` 是当前 **Project Observer retirement release candidate**；本 worktree 已完成源码、文档、Windows 完整回归与 wheel/sdist package smoke，Linux CI、PyPI publish 和 global install 仍需通过后续门禁，本地 release preparation 不等于外部发布。`.92` 从 ACF 核心退役 Project Observer：移除 Observer runtime、Dashboard、Target Registry、semantic-review / presentation lifecycle、narrative 与对应自动化合同子树；`acf observer` 只保留无副作用 `observer_retired` 墓碑，升级不会自动删除用户级 Observer 数据。`v0.0.3.91` 仍是当前稳定安装态（binding-first owner-context 合同），`v0.0.3.90` 是不可移动的失败 release tag，`v0.0.3.89` 是历史 Agent-first Project Observer Beta；canonical Windows `acf` 仍为 `.91`，直到 `.92` 完成外部发布与安装验证。Windows 正式安装态应通过 `scripts/install_acf.ps1` 或 `scripts/update_acf.ps1` canonicalize 为 uv tool bin 下的 `acf.cmd`，同目录不保留 `acf.exe`。
