...
`v0.0.3.37` 是当前推荐的真实项目接入/升级版本，保留 active -> reference 规划依据追溯能力，并新增标准 profile 的 `human/` 人工异步笔记层、统一 `【ACF:KEY|提示】` 模板占位符、canonical ACF marker 和可点击 Markdown 链接维护能力；`archive current-task/task-plan` 会在归档移动时重写本地 Markdown 相对链接。`upgrade` 仍应 dry-run first，Workstream 仍保持显式启用。
