...
`v0.0.3.67` 是当前推荐版本。在 `v0.0.3.66` 的 continuation / semantic-clean hardening 基础上，补齐 worktree close 的最后一跳：只有 ACF 已机械证明 worktree semantic-clean 且存在 content-identical `stat_only_paths` 时，close 才会内部使用 Git 的强制移除参数绕过 raw porcelain 假 dirty；真实 staged/unstaged/untracked/conflict 仍在 remove 前 fail-closed，branch 仍只用安全 `-d` 删除。
