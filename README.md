...
`v0.0.3.23` 是当前推荐的真实项目接入/升级版本，新增 `acf curate draft` 最小版：只消费 `review stale` 的机械信号并生成可审阅注意力治理草案，不判断事实真假、不修改权威上下文。普通 usage log 仍只记录元数据，显式 `acf log feedback` 可记录人工反馈正文。`upgrade` 仍应 dry-run first，Workstream 仍保持显式启用。
...
acf version set v0.0.3.23 --dry-run --json
acf status --json
