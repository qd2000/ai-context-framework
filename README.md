...
`v0.0.3.88` 是当前稳定推荐版本。它在 `.87` 已冻结的 physical-execution liveness 与 graceful handoff 基线上，补齐三个真实 dogfood 缺口：ownerless handoff cleanup 的 evidence-backed `workspace reconcile-handoff`；stale owner challenge 的 tool-backed blocking `coordination wait|await`，使当前 Scheduled activation 能跨 persisted challenge deadline 等待 `owner_active | owner_released | timeout`，而不依赖模型纯 idle、也不把 timeout 偷换成 ownership；以及 `.87` publish 前暴露的 POSIX child-argv、Python 3.10 Observer、Windows path/process race 回归。generated `execution_policy` 明确 pending challenge 不是 session-end signal、wait 不授予 ownership、timeout 后要 fresh reconcile/recover 并在同一 activation 继续安全有价值工作。`v0.0.3.88` 已完成 immutable tag、GitHub release、PyPI wheel/sdist 发布和 Windows 全局安装；安装态 focused continuation 6/6 与 physical-liveness 2/2 回归通过，真实 WS012 也再次由安装态 `coordination wait` 跨 persisted deadline 后 formal reconcile/recover 成功。Project Observer 继续保持 **read broad / write narrow / control none**；下一阶段回到 Human-First Visualization / expected-target Registry recovery，而不是继续扩张 continuation P0。
