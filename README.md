...
`v0.0.3.72` 是当前推荐版本。`acf continuation prompt` 继续采用 goal-directed continuous execution，但把默认注意力进一步收敛到“总体目标 → authority refresh → 当前默认执行计划 → 当前 owner 状态 → 仅与当前状态相关的安全分支”。`state.next_action` 在 refresh 后仍有效时是默认下一步执行计划，而不是只供查看的 hint，也不是 work quota；完成它后若总体目标仍开放且还有安全、有价值工作，Agent 继续自主推进。`prompt --runner-id` 可区分当前 owner、verified live other owner、stale/unverified owner、expired owner 与无 owner；healthy hourly overlap 不再自动 challenge，verified duplicate wake 可以让行结束当前重复 session，但不改变 mission 状态，stale/unverified ownership 才进入 challenge/reconcile/recover。Scheduled Task wrapper 不再追求“薄”，而应是 sufficient high-salience bootstrap：完整携带精确既有 workspace/connector 入口、稳定 ACF 升级适配、generated-plan 执行、owner 用户可见说明和项目专用约束，同时不复制 generic 状态机。`.72` 还让 compact continuation state 的历史摘要列表成为最近 64 项 rolling window，并为 terminal effect history 提供 replay-safe rollover，避免长任务因 state/effect 容量自锁。final response、timing、commit/checkpoint/Gate 的既有非停止语义继续保留。
