...
`v0.0.3.79` 是当前推荐版本。它继承 `.78` 的 Observer Workstream source-authority 修复，并补齐长期 Workstream release 后的 reactivation/sync 生命周期：对已经存在的 registry binding 重新 `worktree attach --apply` 时保留原 Git base/reservation anchors，避免把尚未进入 source branch 的最新 primary HEAD 提前写成 reservation authority，从而消除随后 `worktree sync` 的 `reservation_commit_not_ancestor` 死锁。Project Observer 继续保持 **read broad / write narrow / control none**，structured state/history 使用 UTC，静态 Dashboard 人类可见时间显示北京时间 `UTC+08:00`。
