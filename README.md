...
`v0.0.3.73` 是当前推荐版本。它继承 `.72` 的 sufficient scheduler bootstrap、state-conditioned generated prompt、default execution plan、owner liveness/duplicate-wake 语义和 continuation capacity/effect rollover hardening，并补上一条窄的 legacy local-effect recovery：旧版本若留下 `prepared + external_id=null` 的**本地确定性 effect**，且 owner 已结束/forfeit、durable local authority evidence 已能证明实际 terminal 结果，可显式使用 `reconcile --effect-local-terminal` 收口；该路径不适用于 active/unknown effect，也不能绕过已有 external-id 的 identity-matched reconciliation。ACF 仍不会从文件名或普通项目事实自动裁决 effect 真假，调用方必须提供可审计 evidence，`recover` 仍受 observation digest、receipt 与 generation fencing 约束。
