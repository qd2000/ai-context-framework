...
class ContinuationCliTests(unittest.TestCase):
    ...
    def test_ws008_long_running_profile_configures_existing_task_and_generated_prompt(self) -> None:
        ...
        self.assertIn("does not define a work round, reporting interval, expected stopping point", prompt_text)
    ...
    def test_prompt_is_local_first_and_contains_goal_directed_protocol(self) -> None:
        ...
        self.assertIn("does not define a work round, reporting interval, expected stopping point", prompt)
        ...
        self.assertIn("Do not produce a final response merely to report progress", prompt)
        self.assertIn("Do not infer a session boundary from elapsed time", prompt)
        self.assertIn("Before a final response, identify the concrete factual reason", prompt)
        self.assertIn("explicit signal that the current execution is about to terminate", prompt)
        self.assertIn("release the lease before the final response", prompt)
