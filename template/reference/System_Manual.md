...
`init` 会把当时已有的 changed paths 作为受保护的 external baseline，而不是要求为了自动化先清空整个 worktree。`standard` timing 为 scheduler/TTL/renew/heartbeat/stale=`60/120/30/10/30` 分钟；`long-running` 为 `60/180/45/10/25` 分钟。已有 task 用 `configure` 原位调整，不需要 `init --force`；active lease 存在时 configure 会拒绝执行。每个新 runner 应先用 `coordination attempt` 登记 bounded attempt；已有 owner 时它成为 contender 并自动 open/join 当前 owner generation 的 challenge，但不获得写权限。普通可定位项目的 ACF 命令只在 stderr opportunistic 提示 pending challenge，不是 ACK；只有 owner-protected continuation 命令在 `lease_id + generation + fence_token` 验证后才 authenticated touch，正常 release 则解析为 `owner_released`。challenge timeout 只产生 `ownership_forfeiture_candidate`，仍需 formal reconcile workspace/HEAD/effect/identity 后才允许 generation+1 recover。旧 owner 已结束或 matching ownership-forfeiture 已成立时，`reconcile --effect-key <key> --effect-terminal-status completed|failed --effect-external-id <id> --effect-evidence-ref <ref>` 可对既有 unresolved effect 记录 receipt-bound terminal observation；`external-id` 必须与 prepared record 完全一致，外部权威状态仍 unknown、identity mismatch 或 effect journal/HEAD observation 漂移时继续 fail-closed。该命令不触发 submit/collect/cancel，effect journal 只会在匹配 receipt 的 `recover` 中写入 terminal 结果。current `effects.json` 达到记录/字节边界时，ACF 只把完整 `completed|failed` records rollover 到 bounded `effects.archive.NNNNNN.json` segments；archive 继续参与 logical-key 防重放，`prepared|active|unknown` 永不归档，unresolved 自身耗尽容量时仍 fail-closed。heartbeat 只证明 runner 近期仍活跃，不延长 TTL；stale threshold 与 renew interval 独立；Scheduled Task 的小时触发只是持续任务的恢复唤醒，不是独立工作回合、汇报周期或 Gate 的硬时限。active owner 在写项目文件前用 `workspace intent` 声明 concrete path。workspace manifest v3 记录 bounded path/status/digest，并可保存一次性 legacy adoption receipt：外部修改是 `baseline_external`，当前 task 的显式 intent 修改以及前一 generation 继承的未提交 WIP 是 `task_owned`。worktree-level `clean/dirty` 不参与 continuation 安全判断；正常 release 可以保留 task-owned WIP 并在下一 generation 验证 ownerless handoff digest 后直接继承，所以 scheduler wake 之间不要求 Git commit。无 manifest 的 changed paths 报 `workspace_provenance_missing`，doctor 会提示先审阅再 `workspace adopt`。adopt 必须在没有 active owner 时运行，把当前全部 changed paths 明确列为 `--task-owned` 或 `--baseline-external`，并提供 durable `--evidence-ref` 与 reason；它记录当前 status/content digest、prior generation 和 adoption receipt，bound Workstream 的 task-owned 路径仍受 write_scope 约束。路径遗漏/重叠、HEAD drift、owner identity 不清或来源无法确认时继续 fail-closed。adopt 只补建 workspace provenance，不重置 continuation state、round/effect/coordination history，也不要求先形成 Git semantic checkpoint。同路径/父子路径、task-owned ownerless drift、HEAD/effect/identity ambiguity 仍 fail-closed。`acf continuation prompt` 输出当前 timing 与完整 generic protocol，并且是唯一 generic Scheduled Task protocol 权威；Scheduled Task wrapper 每次 wake 重新读取该 prompt，只保留固定项目身份和项目特有约束。运行态保存在用户级 `~/.acf`，不会写入项目 Git；ACF 不是通用文件编辑器或源码备份系统。

`acf continuation prompt` 采用 goal-directed continuous execution：scheduler wake 只是持续任务的恢复入口，不定义独立工作回合、汇报周期、工作配额或预期停止点；bounded 只约束 ownership、写入、副作用和恢复风险，不限制当前 execution session 的有效工作量。stage/next_action 是 resume context/hint，不是 work quota。generic protocol 不再使用 1→14 顺序 checklist，而按 startup/ownership、contention/recovery、workspace writes、ownership liveness、effects、progress/checkpoints、actual handoff 与 issue reporting 分成 when-applicable 规则组；这些规则不是“走完即可退出”的工作流。final response 会结束当前 execution session，不能为了进度汇报、elapsed time、tool-call count、context length、完成若干步骤、测试/commit/checkpoint/Gate 或主观感觉“该收尾了”而触发。timing 参数只服务 lease liveness，不是 execution-duration target。platform boundary 只有平台、系统或工具明确发出当前执行即将终止的信号时才成立；release 不是默认收尾步骤。任务级硬停止仍只允许总体目标完成、用户明确暂停、需要新的人工授权/凭据/不可替代决策，或项目访问工具经合理重连仍不可用。
...
`continuation prompt --json` 还会返回可机器读取的 `identity` 与 `scheduler_wrapper_contract`，用于生成稳定的模型无关薄 wrapper；wrapper 不复制 generic 状态机，也不再用“本轮收口”“最终汇报”一类 framing 给 scheduler wake 人为制造回合终点。只有 generated protocol 允许结束当前 execution session 时才做 final report。任何可能在 owner/tool-call 结束后继续写项目文件或产生非幂等副作用的 long-lived local subprocess、DevSpace session、Runtime job 或 external job，都必须在启动前通过 durable deterministic effect/job identity 进入控制面，并在可核对 terminal 状态前保持 unresolved；无法持久识别的 detached writer 不得跨 owner 生命周期运行。纯只读 blocking 调用可不建立 writer effect，但返回后、下一次写入或副作用前必须再次 `assert-owner`。

`continuation prompt --json` 除 `identity` 与 `scheduler_wrapper_contract` 外，还返回 `current`、`execution_policy` 和 `project_context`。`execution_policy` 机器字段明确 `scheduler_wake_is_resume_only=true`、`protocol_is_sequential_checklist=false`、`next_action_is_work_quota=false`、`final_response_is_terminal=true`、`final_response_requires_session_end_reason=true`、`progress_report_is_session_end_reason=false`、`elapsed_time_is_session_end_reason=false`、`timing_is_execution_duration_target=false`、`platform_boundary_requires_explicit_signal=true` 与 `release_is_default_end_step=false`。这些字段只描述执行语义，不替代 Agent 的事实判断。

同一 stale generation 若同时存在多个已有 durable external id、且由外部 authority 明确证明 terminal 的 unresolved effects，可在一次 `reconcile` 中按相同顺序重复 `--effect-key`、`--effect-terminal-status`、`--effect-external-id`，以一个 receipt 原子绑定全部 assertions；它们共享本次 `--effect-evidence-ref` 集合，数量、identity 或 digest 不一致继续 fail-closed。write-ahead `effect prepare` 后若尚未真正 submit，仍允许一条单 effect 的显式 no-start 恢复路径：只有 effect 仍为 `prepared`、`external_id=null`，并有外部 authority 证明 submit 从未启动时，owner 结束/forfeiture 后才可用 `reconcile --effect-not-started --effect-terminal-status failed --effect-evidence-ref <ref>` 生成 receipt，并由 `recover` 标记 failed。已有 external id、active/unknown effect、completed 声明或证据不足均继续 fail-closed。
