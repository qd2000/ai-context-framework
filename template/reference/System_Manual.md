...
`acf continuation prompt` 采用 goal-directed continuous execution：scheduler wake 只是持续任务的恢复入口，不定义独立工作回合、汇报周期或预期停止点；bounded 只约束 ownership、写入、副作用和恢复风险，不限制当前 execution session 的有效工作量。stage/next_action 是恢复入口，Agent 自主决定工作范围；子任务、测试、commit、checkpoint 和 Gate 完成都不是停止理由。final response 会结束当前 execution session，不能为了汇报进展、已经运行了一段时间、完成了若干步骤、上下文看起来很长或主观感觉“该收尾了”而触发。platform boundary 只有平台、系统或工具明确发出当前执行即将终止的信号时才成立；另一个 owner、external wait 或 fail-closed 只有在确实阻断当前所有安全、有价值、非冲突工作时才构成 session handoff。任务级硬停止仍只允许总体目标完成、用户明确暂停、需要新的人工授权/凭据/不可替代决策，或项目访问工具（如 DevSpace）经合理重连仍不可用。某个具体动作被安全拒绝时，只禁止该动作，仍应继续安全诊断、证据核查、issue 记录、规划、测试或其他不冲突工作；正常自愿 handoff 时，live owner 应在 final 前完成必要刷新/checkpoint 并 release。
...
`continuation prompt --json` 除 `identity` 与 `scheduler_wrapper_contract` 外，还返回 `current`、`execution_policy` 和 `project_context`。`execution_policy` 机器字段明确 `scheduler_wake_is_resume_only=true`、`final_response_is_terminal=true`、`final_response_requires_session_end_reason=true`、进度汇报与 elapsed time 不是 session-end reason，以及 `platform_boundary_requires_explicit_signal=true`。`project_context` 渲染 continuation state 中的 `plan_refs`、`constraints`，并声明 wrapper 必须填写的项目专用约束槽；wrapper 可补充 tooling、Runtime、resource/VM/node、permission、security、scientific、validation 和 issue-reporting 规则，但不得复制 generic continuation 状态机。
