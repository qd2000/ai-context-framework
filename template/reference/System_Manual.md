...
- `acf archive current-task|task-plan|list|sync [target]`：归档旧当前任务或旧大任务计划，并维护归档索引；归档 current task / task plan 时会按归档文件的新位置重写本地 Markdown 相对链接，并追加 `ACF:ARCHIVE:RECORD` marker；`sync` 从 `archive/tasks`、`archive/plans` 和 `archive/workstreams` 重算 `ACF:ARCHIVE:INDEX-GENERATED` marker block，优先使用归档 record marker 恢复 Task/Plan 归档原因，旧索引首次接入时需显式 `--init-marker`；归档时写入的索引行「原路径」列与 `sync` 渲染同源（Task 为 `active/Current_Task.md`、Plan 为 `active/Task_Plan.md`），不使用占位值，避免归档命令自身让生成块与归档文件不一致。
...
acf continuation workspace review-handoff <worktree> --task-id WS001 --runner-id scheduled-agent --draft-file <local-review.json> --json
acf continuation claim <worktree> --task-id WS001 --runner-id scheduled-agent --handoff-review-file <local-review.json> --json
...
漂移现场仍 dirty 但 Agent 判断值得保留时，用 `workspace review-handoff` + `claim --handoff-review-file` 走"Agent 自行审查 + 原子接管"路径，而不是先 commit / stash / revert / 删除现场：`review-handoff` 只读生成按当前 HEAD 与逐路径 status/digest 绑定的审查包和 `--draft-file` 决策草稿（四种决策：inherit_for_triage / preserve_external / semantic_clean / block，每条 drift path 恰好一次）；`claim --handoff-review-file` 在同一 state lock 内重算 observation digest 并完成全部机械验证后，才应用决策、写 `last_handoff_takeover.json` 审计 receipt、推进 generation 并创建 lease。digest 失配、runner 不一致、scope 冲突、决策未精确解释全部 conflict、HEAD 漂移未显式接受或存在 unresolved effect 都 fail-closed（如 `workspace_handoff_review_stale`、`_runner_mismatch`、`_scope_conflict`、`_conflict_mismatch`、`_head_unaccepted`、`_effects_unresolved`），不建 lease、不改工作区。接管成功只表示"值得保留并继续处理"，不表示内容正确或已测试。
