本文件是项目重要决策索引。

这里不记录完整决策过程，只记录决策摘要、状态和对应 ADR 文件。详细理由请查看 `decisions/` 中的具体 ADR。

---

## 决策状态说明

- Active：当前有效。
- Proposed：已提出，但尚未最终确认。
- Superseded：已被新决策替代。
- Rejected：已否定，不应继续推进。
- Deprecated：不推荐继续使用，但可能仍有历史影响。

---

## 当前有效决策

| ID | 标题 | 状态 | 摘要 | 详情 |
|---|---|---|---|---|
| ADR-0001 | 【决策标题】 | Active | 【一句话摘要】 | `decisions/ADR-0001-template.md` |

---

## 待确认决策

| ID | 标题 | 状态 | 摘要 | 需要确认的问题 |
|---|---|---|---|---|
| ADR-0002 | 【决策标题】 | Proposed | 【一句话摘要】 | 【待确认点】 |

---

## 已替代决策

| ID | 标题 | 状态 | 被哪个决策替代 | 原因 |
|---|---|---|---|---|
| ADR-0000 | 【旧决策标题】 | Superseded | ADR-0001 | 【原因】 |

---

## 已否定方案

| 方案 | 状态 | 否定原因 | 是否允许重新评估 |
|---|---|---|---|
| 【方案名】 | Rejected | 【原因】 | 【是/否，条件是什么】 |

---

## 使用规则

AI 使用本文件时，请注意：

1. 先阅读本索引，再决定是否打开具体 ADR。
2. 不要重复推进已否定方案，除非用户明确要求重新评估。
3. 如果索引和旧 worklog 冲突，以本索引为准。
4. 如果需要理解决策原因，请读取对应 ADR 文件。