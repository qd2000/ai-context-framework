...
- 发布状态：不可变 tag `v0.0.3.100` 指向 `99f49fd`；release workflow #42 先复用 `ci.yml` 作为 publish 前必需门禁（tests：ubuntu/windows × Python 3.10/3.12 与 package smoke：ubuntu/windows 共 6 项作业全绿），`Build, verify, and publish` 作业 success；PyPI `latest=0.0.3.100`（wheel `a649f87c…34461`、sdist `0b667af6…ce84`）；Windows 全局安装经 `scripts/update_acf.ps1` 升级并验证 `acf v0.0.3.100`、`Get-Command acf` 解析到 `uv tool bin` 下的 `acf.cmd`、同目录无 `acf.exe`，且全局安装的 `acf continuation workspace review-handoff --help` 可用。
