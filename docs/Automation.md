...
write-ahead `effect prepare` 与真正 external submit 之间允许一个更窄的 ownerless abandonment 分支：仅当记录仍为 `prepared`、从未取得 `external_id`，且 scheduler/API/Git remote 等外部 authority 能明确证明 submit 未启动时，`reconcile --effect-not-started --effect-terminal-status failed --effect-evidence-ref <ref>` 才能生成 receipt；`recover` 再原子把该 effect 标为 failed。只要 external id 已存在、effect 已 active/unknown、终态想声明 completed，或 no-start 证据不足，就仍 fail-closed。
