...
- Continuation owner 认证统一遵守 [Sensitive Data / Credential Transport Contract](Sensitive_Data_Credential_Transport.md)：`claim/recover` 对外只交付一个绑定 workspace/task/lease/generation/credential 的本地 owner-context capability handle，protected 命令只需该 handle；canonical lease 仍只保存不可逆 verifier。旧 raw-secret/token-file transport 不作为兼容旁路，handle 缺失、错绑定、stale generation 或 credential 不匹配全部 fail-closed，delivery 失败不得提交 fresh owner 或推进 generation。公共 status/doctor/prompt/log 不输出 reusable credential 或无业务必要的 auth verifier；UUID、Git SHA、deterministic digest、generation、普通 external job id 继续作为合法 metadata。
