...
该机制不引入 daemon、数据库或第二套 scheduler，也不允许 replay 已存在的 deterministic key。父 supervisor 保留自己的 owner-context capability 以完成 heartbeat/renew/terminalize，但该 owner capability/credential transport 不委托给 child/descendants。普通环境仍照常继承；这里只做 continuation credential containment，不提供通用 sandbox。公共结果不回显完整 child argv，明确 credential-bearing argv fail-closed，child stdout/stderr 仅作为 bounded diagnostic tail 并在返回前做 credential-like redaction；父命令自身仍遵守 DevSpace command-session terminality：拿到 `running=true/sessionId` 后必须持续 poll **同一个 session** 到明确 exit，不能启动 replacement/retry 或依赖该结果的后续 side effect。worktree candidate 只在隔离 `ACF_HOME` 测试该能力；正式 immutable release/global install 前不得让 candidate 改写 canonical continuation control plane。
...
- `clock now`：输出分钟级本地时间 `YYYY-MM-DD HH:MM`，供 AI 写入上下文时间字段时不手写格式；不依赖上下文目录，支持 `--json`。
...
- `workspace review-handoff` 与 `claim --handoff-review-file` 是 ownerless handoff 漂移的"Agent 自行审查 + 原子接管"路径，不替代上面的 cleanup 收口，也不放宽未知漂移的默认阻塞：`review-handoff` 只读生成按当前 HEAD 与逐路径 status/digest 绑定的审查包，并把决策草稿写到工作区之外的 `--draft-file`，全程不修改、提交、stash、revert 或删除任何工作区文件，也不取得 ownership；`claim --handoff-review-file` 在同一 state lock 内重算 observation digest 并完成后，才应用 `inherit_for_triage / preserve_external / semantic_clean / block` 决策、写 `last_handoff_takeover.json` 审计 receipt、推进 generation 并创建 lease。digest 失配（`workspace_handoff_review_stale`）、runner 不一致、继承路径越出 Workstream direct write scope、external 路径与 scope/intent 重叠、clean 路径仍 dirty、决策未精确解释全部 conflict、HEAD 漂移未显式接受或存在 unresolved effect 都 fail-closed，不建 lease 且不改 workspace。接管成功只表示 Agent 已确认 WIP 值得保留并继续处理（`accepted_for_triage`、`required_initial_action=triage_inherited_wip`），不表示内容正确、完整或已测试；新 owner 仍须先检查、测试并决定继续、修改、拆分、撤销或删除，generated prompt 在该 generation 第一次有效 checkpoint 前保持高显著提示。
...
P1.5 `automation_prompt_execution_contract` v2 保持 Writer `acf.continuation.scheduler_wrapper.v1` 的兼容键不变：Writer static wrapper 继续只保存 sufficient high-salience bootstrap、project/task extension slot 与不可静态固化的 volatile fields；dynamic generic authority 仍唯一来自每轮 `acf continuation prompt + execution_policy`。`v0.0.3.92` 起该 contract 不再包含 Production Observer wrapper、observer semantic review、observer execution evidence 或 presentation maintenance 子树；`writer_runtime_prompt_contract.must_not_embed` 仍保留历史负向 token，用于继续禁止把已退役的 Observer 状态机嵌入 Writer runtime prompt。
