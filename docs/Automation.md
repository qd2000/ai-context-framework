...
P1.5 将上述关系进一步固化成 `automation_prompt_execution_contract`，并保持现有 `acf.continuation.scheduler_wrapper.v1` 兼容键不变。Writer static wrapper 明确 `wrapper_family=writer / role=maintenance_writer`、命名的 `project_task_specific_constraints` extension slot、不可静态固化的 owner/generation/stage/next_action/directive/release/acceptance 等 volatile fields，以及 Observer 真正有用的 semantic stage / milestone / major outcome / proven / excluded / blocker / **problem** / **plan impact** / route impact / next logic evidence contract；dynamic generic authority 仍唯一来自每轮 `acf continuation prompt + execution_policy`。Production Observer static wrapper 明确 existing-checkout、显式 Target Registry、`read broad / write narrow / control none`、anti-masking、truthful run history、Map Review Gate、human-first-but-detailed 与 project/target extension slot，并禁止复制 Writer ownership state machine。每次 semantic refresh 都必须显式选择 `unchanged|patch|rebuild|presentation_change|project_overview_enable|project_overview_disable`，附 reason/evidence/source fingerprint；fingerprint 只能做 triage，semantic-risk change 必须 authority reread。interactive presentation maintenance 只允许 `transient_patch / one_shot_semantic_review / durable_rule` 三种 lifecycle。任何交互写在持久化前都要求 Agent 先理解当前 derived semantic/presentation state，并整理 `reviewed_intent / scope / rationale / evidence_refs`；ACF 只做确定性 record validation 与 concurrency 校验，不替 Agent 判断语义。即时 patch 只能经 user-level narrow derived-presentation interface 写 presentation-only state，并要求 deterministic re-render；不得直接改 `dashboard.html` 或 semantic truth。one-shot 在下一次 Production Observer 成功消费后退出 active context且不得复活；durable rule 才长期保留并支持 supersede/withdraw。所有交互写继续遵守 `read broad / write narrow / control none` 与 foreign-project non-interference，并采用 presentation revision/fingerprint optimistic concurrency；冲突时 fail-closed reread/re-evaluate，不参与 Writer ownership。

P2 将 P1.5 中已经冻结的 semantic-review / one-shot / durable-rule 语义落实到用户级 Observer runtime。`acf observer presentation-status` 只读计算每个显式 Target Registry target 的独立 source fingerprint 与 `not_reviewed|current|stale`，并返回 active one-shot request / durable rule；target A 变化不会让 target B 一起 stale。`review-request-add` 记录下一次 Production Map Review 必须处理的一次性 reviewed intent；`semantic-review-apply` 只有在 exact target revision、exact target source fingerprint、authority fingerprint、reason/evidence 都匹配时才写入，map-relevant signal 存在时还要求显式 authority reread。成功消费的一次性 request 立即退出 active context，只在 audit event 中保留 terminal evidence，后续 Production render 不得复活。`presentation-rule-add` 只用于明确 durable guidance，可通过 `--supersedes` 替换旧 active rule；`presentation-rule-withdraw` 明确取消而不删除历史。target narrative 记录 goal/route/current-position/why-now/recent-proof/next-logic 与 route nodes/edges；problem model 分别记录 expectedness、handler、scope relation、blocking impact、plan impact 与 lifecycle，route-changing problem 禁止配合 `decision=unchanged`，foreign-project handler 只允许记录 transfer linkage。所有这些写仍是 `read broad / write narrow / control none` 的 derived state，不取得 Writer ownership、不改项目 Git/Markdown authority。即时 `transient_patch` 与 deterministic re-render 仍属于 P3，不在 P2 伪造 acceptance。

P3 将 Dashboard 重建为中文 human-first、自包含 target 页面，并正式实现 narrow `transient_patch`。页面优先展示最终目标、完整路线、当前位置/why-now、最近证明或排除、problem + plan impact、next-logic，再保留当前执行范围、run chain、Alerts 与 Timeline。`transient-patch-apply|clear` 只写 user-level derived presentation state；每次写入必须同时匹配 exact target revision、presentation revision 与 presentation fingerprint，并且只能针对已存在的 canonical `current.json` target view。apply/clear 完成后对**同一个 canonical snapshot** deterministic re-render Dashboard，不追加 snapshot/run，不触碰 Writer control plane；这样 Maintenance/interactive presentation 调整不能把比 Production Observer 更新的事实偷偷渲染进旧 snapshot。缺少 canonical current、presentation view 冲突、semantic source drift 或 semantic-risk signal 均 fail-closed；semantic-risk 必须升级到正式 Map Review + authority reread。新的 semantic review 会使 active transient patch 自动过期，clear 后不得复活；与 P2 的 one-shot consume-once、durable rule supersede/withdraw 一起构成三 lifecycle integrated acceptance。
