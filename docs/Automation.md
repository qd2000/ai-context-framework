...
- `continuation init|configure|list|migrate|doctor|coordination status|coordination attempt|coordination challenge|claim|assert-owner|heartbeat|progress|effect prepare|effect update|effect list|workspace status|workspace intent|workspace adopt|workspace refresh|reconcile|recover|renew|checkpoint|release|pause|resume|prompt|issue`：为外部 AI scheduler 或人工多轮任务提供模型无关的 bounded continuation 控制面。时序参数拆分为 scheduler interval、lease TTL、renew interval、heartbeat recommendation 与 stale threshold；`long-running` profile 默认 `60/180/45/10/25` 分钟，已有 task 可用 `continuation configure --profile long-running` 原位切换。每个 runner 先登记 bounded attempt；已有 owner 时 contender 自动 open/join challenge，但 challenge 本身不授予写权限。普通可定位项目的 ACF 命令只 opportunistic 提示当前 owner generation 的 pending challenge，绝不充当 ACK；owner-protected continuation 命令只有在 `lease_id + generation + fence_token` 校验成功后才 authenticated touch，release 则解析为 `owner_released`。challenge timeout 仅是 `ownership_forfeiture_candidate`，formal preemption 仍必须消费 workspace/HEAD/effect/identity reconcile evidence，并由 generation fencing 保证最多一个 writer。ownerless recovery 如已有外部权威证明某个**既存 deterministic effect identity** 已 terminal，可在 `reconcile` receipt 中显式携带 `--effect-key/--effect-terminal-status/--effect-external-id/--effect-evidence-ref`；该路径只记录 observation，不触发外部动作，不允许新建 effect 或替换 external id，并把 receipt 绑定 observed effect digest，只有后续 `recover` 仍完全匹配时才 terminalize journal，unknown、identity mismatch、observation drift 继续 fail-closed。workspace manifest v3 使用 bounded Git path/status/digest 管理 `baseline_external / task_owned / external non-overlap / conflict`，并可保存一次性 legacy adoption receipt；worktree-level `clean/dirty` 不参与 liveness、claim、release、reconcile、recover 决策。正常 release 可保留未提交 task-owned WIP 并 handoff 到下一 generation，Git commit 只服务自然语义 checkpoint；ownerless handoff 期间 task-owned digest 漂移才产生真实 conflict。一般 `baseline_external` 仍禁止 stash/reset/clean/stage/commit；唯一窄例外是当前 authenticated runner 刚刚通过 `acf workstream scope-add|merge-request|ready|merge-start|done` 为**绑定 Workstream**产生的两个确定性 authority 文件：`docs/ai/active/Workstreams.md` 与 `docs/ai/active/workstreams/` 下当前绑定 Workstream 的 detail 文件。在生命周期/merge 边界，Agent 可以先审阅 exact diff 与 durable ACF command evidence，再单独形成 control-plane checkpoint；该例外不扩展 Task authority write scope、不得带入第三个 baseline/external path。无 manifest 的 changed paths 报 `workspace_provenance_missing`；人工或 Agent 审阅后可在没有 active owner 时运行 `workspace adopt`，必须把**当前全部 changed paths**逐项显式归为 `--task-owned` 或 `--baseline_external` 并提供 evidence refs，记录 status/content digest/prior generation，task-owned adoption 继续受 Workstream write_scope 约束。遗漏/重叠路径、HEAD drift、active/unknown owner 或无法确认来源都 fail-closed；adoption 仅补建 workspace manifest，不重置 state/round/effect/coordination history，也不要求先提交 Git。Scheduled Task 的小时触发只是唤醒，不截断自然工作范围；`acf continuation prompt` 是唯一 generic Scheduled Task protocol 权威，项目 wrapper 每轮重新读取它，只保留固定身份和项目特有约束。generated prompt 默认采用 goal-directed continuous execution：bounded 只限制 ownership、写入、副作用和恢复风险，不限制一次 activation 的工作量；stage/next_action 是恢复入口，Agent 自主决定工作范围；子任务、测试、commit、checkpoint 或 Gate 完成都不构成退出理由。任务级硬停止只允许总体目标完成、用户明确暂停、需要新的人工授权/凭据/不可替代决策，或项目访问工具（如 DevSpace）经合理重连仍不可用。运行态只保存在用户级 `~/.acf/projects/.../continuation/`，ACF 不保存 transcript/raw tool output、不创建定时任务、不运行常驻 agent。设计见 `docs/ai/reference/Continuation_Control_Design.md`。

- `observer status|snapshot|interpret|glossary-set|glossary|history`：提供项目级 Global Observer。一个项目对应一个用户级 Observer namespace，同项目 primary checkout 与 ACF 注册 worktree 统一进入观测域；边界固定为 **read broad / write narrow / control none**。Observer 可广泛读取 Git、Workstream、continuation、计划和 evidence，但只把 current/history/self-health/semantic/glossary 与自包含 `dashboard.html` 写入 `~/.acf/projects/<project-id>/observer/`，不 claim/challenge/recover continuation，也不修改 Writer 项目文件。snapshot 采用稳定重读、一致性 fingerprint、credential-like 值过滤、轻量锁与原子替换；meaningful history 默认永久保留，只做无损按月 rotation/index。语义解释由 AI 生成，但必须绑定当前 `source_fingerprint`，CLI 负责 canonical identity、confidence、provenance、版本历史与 stale-cache fail-closed；缺少明确分母时禁止假百分比。静态 Dashboard 只渲染 Observer state，不启动 HTTP/daemon；颜色同时配文字/符号，字体保持连续阅读尺度，渲染失败保留 last-good 文件。Observer Core 不硬编码 MCP、电脑、盘符或项目实例；具体项目访问工具只属于 scheduler wrapper/adapter。新 Observer 能力先在 ACF 自身使用 worktree 内开发态 CLI 连续 dogfood，确认高优先级问题收口后才进入全局/PyPI 发布链路。

薄 wrapper 的机器合同由 `continuation prompt --json` 的 `identity`、`current`、`execution_policy`、`project_context` 与 `scheduler_wrapper_contract` 暴露：scheduler 固定 identity，每轮重新获取 generated prompt；`execution_policy` 明确目标导向持续执行、Agent 自主范围、checkpoint/commit/Gate 非停止语义和四类任务级硬停止条件；`project_context` 渲染 state 中的 `plan_refs`、`constraints`，并提供必须由 wrapper 填写的项目专用约束槽。wrapper 只附加 tooling、Runtime、resource/VM/node、permission、security、scientific、validation 和 issue-reporting 约束。long-lived local subprocess、DevSpace session、Runtime job 或外部 job 只要可能在当前 owner/tool-call 返回后继续写项目或产生非幂等副作用，就必须先 `effect prepare` 并持久记录可复用 external/job identity；直到权威终态前都保持 `prepared|active|unknown`，从而阻止不安全 recovery。没有 durable identity 的 detached writer 不属于自动恢复支持范围。纯只读长阻塞调用可不登记 writer effect，但返回后、下一次写入或副作用前必须重新校验 owner。

durable writer 已权威结束但在启动前遗漏真实输出 intent 时，输出先保持 `unexpected_nonoverlap`，不能由后续 refresh 静默归入 task。当前 fenced owner 审阅来源并提供 durable evidence 后，可使用 `continuation workspace reclassify --task-owned <path> --evidence-ref <ref> --reason <review>` 显式纳入任务，或用 `--baseline-external <path>` 明确保留为外部修改。reclassify 只接受当前 unexpected path，task-owned 仍受 Workstream direct write scope 约束；conflict、路径重叠、scope 越界、证据缺失或 provenance 不确定继续 fail-closed。

同一 stale generation 同时残留多个已有 durable external id、且已由 authority 证明 terminal 的 unresolved effects 时，`reconcile` 可按相同顺序重复 `--effect-key`、`--effect-terminal-status` 与 `--effect-external-id`，用一个 receipt 原子绑定全部 assertions；`--effect-evidence-ref` 是该次多 effect observation 的共享 authority evidence 集合。各组数量、external identity 或 observation digest 任一不匹配都继续 fail-closed。write-ahead `effect prepare` 与真正 external submit 之间仍保留更窄的 ownerless abandonment 分支：仅当单个记录仍为 `prepared`、从未取得 `external_id`，且 scheduler/API/Git remote 等外部 authority 能明确证明 submit 未启动时，`reconcile --effect-not-started --effect-terminal-status failed --effect-evidence-ref <ref>` 才能生成 receipt；`recover` 再原子把该 effect 标为 failed。只要 external id 已存在、effect 已 active/unknown、终态想声明 completed，或 no-start 证据不足，就仍 fail-closed。
