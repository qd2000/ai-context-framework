...
- `continuation init|doctor|claim|assert-owner|heartbeat|progress|effect prepare|effect update|effect list|workspace status|workspace intent|workspace refresh|reconcile|recover|renew|checkpoint|release|pause|resume|prompt|issue`：为外部 AI scheduler 或人工多轮任务提供模型无关的 bounded continuation 控制面。新 round 用递增 generation + owner fence credential 保护写所有权；只持久化 fence hash。write-ahead effect journal 禁止 uncertain replay。workspace manifest 只保存 bounded Git path/status/digest metadata：初始化时已有 dirty 作为 `baseline_external` 受保护，owner 在写文件前声明 concrete write intent，绑定 Workstream 时 intent 必须命中直接 write scope；refresh 把当前 dirty 分为 runner-owned、非重叠 external 和真实 conflict。非重叠 external dirty 保留且不自动提交，不再等同于另一个 agent；runner-owned/conflicting dirty 未 checkpoint 时 release 仍 fail-closed。`reconcile` 把 runner-liveness、Git、round/effect journal、workspace ownership digest 与调用方明确 evidence assertion 合成可审计 decision；dirty 本身不阻塞 stale/orphan recovery，只有 manifest 缺失/无效、generation 漂移、真实 path conflict、HEAD/effect/identity 无法解释才 fail-closed。`recover` 只消费 eligible receipt，并在写新 lease 前重新比对整份 observation；任何旧 runner resurrection、workspace digest 变化或本地状态漂移都会返回 stale reconciliation。成功时 generation+1 fencing，同时继承旧 generation 的 write intents 和可证明 `runner_owned` WIP，继续保护不重叠 external dirty，不 stash/reset/clean，也不自动提交 external dirty。wrong task-id 会列出 available tasks。运行态保存在用户级 `~/.acf/projects/.../continuation/`，不写入项目 Git；ACF 不保存 transcript/raw tool output，不创建定时任务、不运行常驻 agent。Scheduled Task 应保留项目 wrapper，并在每轮读取 `acf continuation prompt` 作为 generic protocol 权威。设计见 `docs/ai/reference/Continuation_Control_Design.md`。
