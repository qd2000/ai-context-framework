...
1. 可安装命令与上下文自动发现：让 `acf` 可以在任意项目子目录中使用。
2. `acf status` 和 `--json` 输出：让 AI 能稳定判断当前上下文位置、状态和下一步。
3. `--dry-run` 与写后检查：让写操作可预览、可验证、可回退。
4. 根薄入口自定义字段：允许用户在生成时追加少量仓库级规则，但仍不把完整上下文写入根入口。
5. 安全结构化编辑原语：section get/replace/append 和 table upsert。
6. `writeback-curator` 接入：由 subagent 生成更高质量的回写分类草案，但仍只输出草案。
...
5. 新增或重置当前任务时优先使用 `new task`，新增资料索引时优先使用 `new source`，重要设计决策优先使用 `new adr`，当天工作记录优先使用 `new worklog`。
6. 会话结束回写建议需要暂存时，优先使用 `writeback draft`，再由人或主代理审阅后决定是否写入权威上下文。
7. 修改 `template/` 前先确认该变更属于通用产品模板需求，而不是本仓库 dogfooding 特例。
8. 如果发现跨文件同步问题，优先考虑补充 `acf.py check` 规则，而不是只补文档说明。
9. 如果某项维护动作重复出现两次以上，评估是否应新增 CLI 子命令或 subagent 草案流程。
