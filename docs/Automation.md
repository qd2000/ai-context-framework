...
Agent-owned 文本稳定入口晋升前还必须做最小内容完整性检查：候选需要能够以 strict UTF-8 回读，且不得包含 Unicode replacement character（U+FFFD）。但 successful UTF-8 decode / U+FFFD=0 不能单独证明可见文本未发生 mojibake；Agent 还要从当前 authority/source 选取预期可见文本 marker（例如项目原生非 ASCII 标签或等价 sentinel）并在晋升前回读确认。若实际命令/PTY 通道的 Unicode 保真未经证明，则写入稳定候选时必须使用 byte-preserving 路径或 ASCII-safe payload transport。任一检查失败都视为普通更新失败并继续保留 last-good；这只是输出完整性保护，不把 ACF 扩张成 renderer、通用 HTML validator 或特定语言检测器。
