...
- `continuation init|doctor|claim|assert-owner|heartbeat|progress|effect prepare|effect update|effect list|reconcile|recover|renew|checkpoint|release|pause|resume|prompt|issue`：为外部 AI scheduler 或人工多轮任务提供模型无关的 bounded continuation 控制面。新 round 用递增 generation + owner fence credential 保护写所有权；只持久化 fence hash。write-ahead effect journal 禁止 uncertain replay。`reconcile` 把 runner-liveness、Git、round/effect journal 与调用方明确 evidence assertion 合成可审计 decision：fresh owner 始终 blocked，stale/legacy active 必须显式证明 owner 已结束，HEAD advance 必须接受精确 current HEAD，unresolved effect 仍 blocked；`--record` 才写 user-level reconcile receipt。`recover` 只消费 eligible receipt，并在写新 lease 前重新比对整份 observation；任何旧 runner resurrection 或本地状态漂移都会返回 stale reconciliation，成功时 generation+1 fencing 并写 recovery receipt。clean release 默认保留显式 checkpoint 的非 running state，只有仍为 running 时才回 ready；wrong task-id 会列出 available tasks。`continuation issue --resolve-fingerprint` 以 append-only resolution event 关闭已修复 dogfood issue，`log issues --open-only` 过滤 resolved，后续同 fingerprint occurrence 自动 reopen。旧 v0.0.3.61 无 heartbeat/generation/journal 的 task 可原位恢复，不需要 force re-init。运行态保存在用户级 `~/.acf/projects/.../continuation/`，不写入项目 Git；ACF 不保存 transcript/raw tool output，不创建定时任务、不运行常驻 agent。设计见 `docs/ai/reference/Continuation_Control_Design.md`。
