...
- `continuation init|configure|list|migrate|doctor|coordination status|coordination attempt|coordination challenge|claim|assert-owner|heartbeat|progress|effect prepare|effect update|effect list|workspace status|workspace intent|workspace adopt|workspace refresh|reconcile|recover|renew|checkpoint|release|pause|resume|prompt|issue`：为外部 AI scheduler 或人工多轮任务提供模型无关的 bounded continuation 控制面。既有 ownership/generation fencing、workspace provenance、effect identity、reconcile/recover 与 handoff 安全语义保持不变。`long-running` profile 默认 scheduler/TTL/renew/heartbeat/stale=`60/180/45/10/25` 分钟，但这些 timing 只服务 lease liveness，不是 execution-duration target、work quota 或汇报周期。Scheduled Task wake 只是持续任务的恢复入口；`acf continuation prompt` 是唯一 generic protocol 权威，项目 wrapper 每次 wake 动态读取它，只保留固定 identity 和项目特有约束。`.69` generated prompt 不再把 generic protocol排成 1→14 的顺序工作流，而改成 startup/ownership、contention/recovery、workspace writes、liveness、effects、progress/checkpoints、actual handoff、issue reporting 等 when-applicable 规则组，明确“读到最后一节”不产生结束信号。stage/next_action 只是 resume context/hint，不是工作配额；Agent 继续自主决定工作范围、顺序、实现策略和验证深度。final response 会终止当前 execution session，因此不能只为进度汇报、elapsed time、tool-call count、context length、完成若干步骤或测试/commit/checkpoint/Gate 而触发；release 也不是默认流程最后一步。只有当前确实没有可继续的安全、有价值、非冲突工作，或平台/系统/工具明确发出即将终止信号时，才执行实际 handoff。任务级硬停止仍只允许总体目标完成、用户明确暂停、需要新的人工授权/凭据/不可替代决策，或项目访问工具经合理重连仍不可用。
...
薄 wrapper 的机器合同由 `continuation prompt --json` 的 `identity`、`current`、`execution_policy`、`project_context` 与 `scheduler_wrapper_contract` 暴露。`execution_policy` 在既有字段上新增/稳定表达：`scheduler_wake_is_resume_only=true`、`protocol_is_sequential_checklist=false`、`next_action_is_work_quota=false`、`final_response_is_terminal=true`、`final_response_requires_session_end_reason=true`、`progress_report_is_session_end_reason=false`、`elapsed_time_is_session_end_reason=false`、`timing_is_execution_duration_target=false`、`platform_boundary_requires_explicit_signal=true`、`release_is_default_end_step=false`。这些字段描述执行语义，不替代 Agent 对项目事实和目标完成度的判断。
