...
Agent-owned 文本稳定入口晋升前还必须做最小内容完整性检查：候选需要能够以 strict UTF-8 回读，且不得包含 Unicode replacement character（U+FFFD）。该检查失败视为普通更新失败，继续保留 last-good；这只是输出完整性保护，不把 ACF 扩张成 renderer 或通用 HTML validator。
