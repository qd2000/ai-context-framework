...
- `continuation init|doctor|claim|assert-owner|heartbeat|renew|checkpoint|release|pause|resume|prompt|issue`：为外部 AI scheduler 或人工多轮任务提供模型无关的 bounded continuation 控制面。新 round 用递增 generation + owner fence credential 保护写所有权；只持久化 fence hash，owner credential 不得写入项目文件或普通日志。`heartbeat` 更新 liveness 但不延长 TTL，`renew` 同时刷新 heartbeat/renew 时间并延长 TTL；`doctor` 把 stale active lease 暴露为 `orphan_candidate`，但仍保持不可 claim，后续必须由正式 reconciliation/recover 证明接管安全。旧版无 heartbeat/generation 的 lease 以 `legacy_unknown` 保守兼容。运行态保存在用户级 `~/.acf/projects/.../continuation/`，不写入项目 Git；ACF 只管理 worktree 身份、单写者 lease、compact current state、暂停和 fail-closed 恢复，不创建定时任务、不保存 transcript、不运行常驻 agent。已有 clean worktree 可直接接入，不需要因为 continuation 重建 worktree。设计见 `docs/ai/reference/Continuation_Control_Design.md`。
