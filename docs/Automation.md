...
- `continuation init|doctor|claim|assert-owner|heartbeat|progress|effect prepare|effect update|effect list|renew|checkpoint|release|pause|resume|prompt|issue`：为外部 AI scheduler 或人工多轮任务提供模型无关的 bounded continuation 控制面。新 round 用递增 generation + owner fence credential 保护写所有权；只持久化 fence hash，owner credential 不得写入项目文件或普通日志。`progress` 保存 bounded generic phase/milestone/evidence refs；`effect prepare` 在外部/non-idempotent 写入前记录 deterministic logical identity，`effect update` 只追加 compact durable observation，`effect list` 供恢复轮次核对。prepare 返回 `created=false` 时禁止再次执行该 logical effect；expired running round 一旦已有 effect record，当前版本直接 fail-closed 为 `effect_reconciliation_required`，不沿用旧式 clean-HEAD 自动 re-claim，直到正式 reconcile/recover 能证明安全。`heartbeat` 更新 liveness 但不延长 TTL，`renew` 同时刷新 heartbeat/renew 时间并延长 TTL；旧版无 heartbeat/generation/round/effect journal 的目录继续 lazy/保守兼容。运行态保存在用户级 `~/.acf/projects/.../continuation/`，不写入项目 Git；ACF 不保存 transcript/raw tool output，不创建定时任务、不运行常驻 agent。设计见 `docs/ai/reference/Continuation_Control_Design.md`。
