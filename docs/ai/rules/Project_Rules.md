...
- 维护 `docs-acf/ai/` 内已有 heading 或 table 的上下文文档时，优先先用 `uv run acf edit ... --dry-run --json` 预览，再执行正式写入。
- `acf edit` 只做确定性结构化落盘，不裁决事实内容；写入前后仍需要由人或主代理审阅语义是否正确。
- `acf edit` 当前只允许编辑上下文根目录内已有 `.md` 文件；[../Automation.md](../../Automation.md) 等 `docs-acf/ai/` 外仓库级文档暂时使用常规补丁，直到项目级安全编辑边界完成设计。
...
- 修改 `template/` 前先判断该内容是否属于可复用产品模板；本仓库 dogfooding 专属要求应写入根入口、`docs-acf/ai/` 或维护文档。
