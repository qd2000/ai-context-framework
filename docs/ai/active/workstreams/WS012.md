...
`v0.0.3.86` 已完成正式 release/global install。continuation physical-execution/self-stale P0 的实现范围现已按最新 priority-100 authority 冻结在 checkpoint `8081983f23cee184f32c8028caa6185d521d5b30`：exact PID + process-start identity、same-generation physical liveness、keepalive/renew、terminalization、PID reuse/dead/zombie/unverifiable/expired-lease rejection、scoped process-tree cleanup 与 deterministic replay protection 已形成 focused evidence；不再无直接 failing invariant 地扩展 P0，也不再为了 sequencing 单独发布一个 P0 patch。issue `d3456d0b5cf5c261652d` 的 terminal resolve 仍要求后续 installed-state proof。

当前最新实现主线已切换为 durable directive `dir-d98bf9b8628244cabe68` + `dir-fb0de0b1b4fc4536b97a` 定义的 **Scheduled Writer overconstraint reduction / graceful handoff**：控制面检查要与 evidence-backed risk 成比例；wall-clock/timing 只作诊断，不固化成执行预算；active lease、历史 abnormal generation、task-owned dirty、scheduler wake、checkpoint/test/commit/Gate 或单 lane blocked 本身都不是 stop signal；verified-live physical execution 继续严格禁止 duplicate；genuine safe session end 要持久化 progress/next_action/evidence 后释放 owner 并保持 long-lived mission `running`，避免人为制造 stale/ghost owner。prompt/JSON 同步提供 bootstrap/control-plane、project work、physical execution、graceful handoff、abnormal/incomplete termination、recovery overhead 的机器可读 observability。P0 checkpoint 与本阶段改动合并进入下一正常 stable release/global install/installed-state dogfood。

durable directive `dir-5272efd83a0e434298e1` 已成为 P0 之后的强制路线 authority：既有 P3 只保留为 machine/runtime checkpoint，不算 human acceptance；进入广泛 P4 或最终 Production acceptance 前，必须完成 **Human-First Visualization Gate** 与 **expected-target Registry recovery Gate**。Dashboard 要中文优先、真实展示项目路线/关系/当前位置/问题/下一步/run outcome，并通过真实 render 的迭代 human-first rubric；FCC 必须从 fresh authority 幂等恢复用户授权的 WS079/WS080/WS086 targets，意外空 Registry/expected target 缺失必须 fail-visible degraded/alert，不能显示绿色健康。
