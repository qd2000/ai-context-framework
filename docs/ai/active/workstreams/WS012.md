...
`.88` release / global install / continuation closeout 已完成并禁止 replay；详细产品 authority 以 `OBSERVER_HTML_VISUALIZATION_PLAN.md` 与 `PLAN.md` 为准。当前 formal stage 仍是 **H1 Map-First Visual Gate**，但 2026-09-10 的人工复核已经明确 FAIL，且最新 priority-100 directive `dir-41b8581a296544399208` 已把后续目标升级为 **Task-Semantic Visualization v1.2**：不再把 generic roadmap/narrative/card page 当 H1 终点，而是由 Production Observer 从 fresh authority 判断 target 的核心进展问题，再由 Agent 在 `metric_trend / status_matrix / process_flow / compact roadmap` 中选择一个 dominant primary visualization；ACF 只做通用验证和确定性渲染，禁止按项目/target 硬编码。预期 dogfood 为 WS086 objective trend、WS079 component matrix、WS080 真实 FCC/建模 process flow、WS012 compact roadmap，实际选择仍须由 fresh authority/evidence 决定。现有 expected-target、run provenance/history、archive/concurrency、anti-masking 等已验证 WIP 保留；新的人工 PASS 前 H1 不关闭、formal H2 不启动。

此前 g121–g138 的关系图、双视口、isolated terminal baseline、历史版本、run provenance/route/evidence coverage、expected-target conflict/recovery、graceful-handoff truthfulness 与 storage/module split 均作为当前 classified WIP 基线保留。当前大规模未提交 WIP 必须先在最早安全语义边界完成 validated Git checkpoint，再进入 v1.2 最小 `primary_visualization` contract 和 specialized renderers；checkpoint 本身不等于视觉 PASS。

generation 137 已在 g136 stale challenge 正式超时、fresh doctor 证明 physical execution absent / HEAD unchanged / workspace 全分类 / unresolved effects=0 后，通过 receipt-backed reconcile/recover 合法接管。接管后四个 Observer focused 模块重新 **119/119 PASS**；随后对 H1 中文优先主展示做静态审查，修复已知 `future` route/milestone 状态仍可能直接显示英文的问题，主图现在显示“后续阶段”，并补齐 relation-diagram 常见 fresh/stale/not-reviewed/unconfigured 状态中文映射。`tests.test_observer_presentation + tests.test_package_skeleton` 合计 **47/47 PASS**。该增量只收紧 human-first presentation，不改变 route/authority semantics，**H1 仍需真实视觉 rubric 才能关闭**。

generation 138 在 g137 stale 后通过 tool-backed challenge wait 跨过 persisted deadline，并基于 fresh physical-execution absent / unchanged HEAD / classified workspace / unresolved effects=0 evidence，以 reconcile receipt `17cb71a8-3c1d-43b9-b234-2b597549a972` 正式 recover。随后继续 H1 中文优先静态审查：SVG 节点 tooltip 不再显示 raw machine status；问题卡片把 handler、blocking impact、plan impact 与 lifecycle status 映射为中文并新增明确“阻塞影响”字段；关系图对当前真实常用的 `next / observed_by` 等已知 machine relation enum 使用中文箭头标签，未知自由文本仍原样 fail-visible。最新 relation regression 1/1、presentation+package-skeleton 47/47 均 PASS；此前同代四个 Observer focused 模块 119/119 PASS，`observer_storage.py=1996` 行仍满足 package size Gate。**H1 formal stage 仍保持 open，不能以本轮 machine evidence 代替真实视觉验收。**
...
当前 candidate scope 进入 Observer Task-Semantic Visualization v1.2：H1 仍是 formal Gate，先保留既有关系图、expected-target、run/history/archive/concurrency 等通用能力作为 fallback 与证据层，再新增最小 `primary_visualization` contract 和四类通用 specialized renderer；不同 target 根据 fresh authority 选择不同主图，generic narrative 不再是目标终点。完成真实 ACF + FCC candidate 页面并取得人工 PASS 后，才正式进入后续 H2/H3/H4/H5。所有 candidate dogfood 继续使用隔离 `ACF_HOME`，不写 canonical Production Observer runtime。
