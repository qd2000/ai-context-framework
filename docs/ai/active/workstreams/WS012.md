...
`.88` release / global install / continuation closeout 已完成并禁止 replay。当前详细产品 authority 已切换到 `OBSERVER_HTML_VISUALIZATION_PLAN.md` 的 **Observer Agent-first v2.1**，`PLAN.md` 只保留当前路线与指针。priority-100 durable directive `dir-153b33e6d0cb4bccbb67` supersede 旧 `dir-41b8581a296544399208` v1.2 fixed-renderer 后续路线：Project Observer 的主体是自主 Agent，ACF 降为可选导航/数据/维护工具和参考材料，不再作为唯一事实来源、页面生成前置或固定 renderer。新的 priority-100 durable directive `dir-e8d265cd219943b3b702` 只细化 Beta sequencing，不 supersede Agent-first 主路线：minimum S2 后尽快发布 early Beta，用户手工迁移现有三个 Production Observer，真实 dogfood 后继续 S2/S3/S4/S5 并发布 successor stable，再由用户手工更新/复核。

当前阶段已从旧 H1 renderer 扩展路线切换到 **S1→minimum S2→Beta preparation**：S0 authority/directive 主路线一致性已经落盘，generation 166 已直接用真实 WS012/ACF 资料生成首份 Agent-authored candidate 页面 `output/observer/candidate/index.html` 和简明接续记录 `output/observer/OBSERVER_NOTES.md`。该生成链没有依赖 `primary_visualization.kind/spec`、Map Review/presentation lifecycle、`acf observer snapshot` 或 ACF deterministic renderer；旧关系图、expected-target、run/history/archive/concurrency 等已验证成果只作为兼容数据/参考，不作为新页面必须经过的状态机。静态自包含检查、file-scoped guard 与 strict docs check 已通过，但真实浏览器视觉/交互和用户人工 PASS 仍未取得。generation 167 已正式接管 generation 166 的 minimum-S2 automation contract WIP，并先处理最新 Beta sequencing authority。

真实性、中文优先、北京时间、历史可查、真实指标口径、人工视觉验收、Production Observer anti-masking、Writer continuation/effect/Git safety 全部保留。人工视觉 review pending 不再阻塞可逆解耦、历史/数据补足和其他有价值工作。两条 priority-70 context/template 简化需求继续有效，其中与 Observer 大减法直接相关部分并入当前路线，其余继续作为 Maintenance 范围，不按年龄自动关闭。

当前 next action：完成并验证 generation 166 留下、generation 167 已正式接管的 minimum-S2 automation contract WIP；优先核对 `automation_contracts.py` 与 Observer presentation/render/storage 的真实调用者，把 explicit Registry display gate、每轮 semantic-review/Map Review、fixed renderer 等从新 Observer 的必经前置降为 optional/legacy，并确保旧 snapshot/maintenance 不覆盖 Agent-owned candidate/latest，同时保留 Writer continuation ownership/effect/fencing、真实 run/history 与 Production anti-masking。通过 focused regressions 后立即进入 Beta release preparation：核验下一 immutable version identity（`.89` 只是未占用时的预期）、准备 release checks；Beta/global install 后 Scheduled Task prompt 更新由用户按正式计划中的 copy-paste-ready 提示包和 checklist 手工执行。不得把当前原型人工视觉 PASS、穷尽 legacy 或完整 S3 当作 Beta 前置；不得先设计新的通用 renderer/API/schema/plugin platform，也不得通过 Maintenance snapshot 刷新 canonical Production Dashboard。
