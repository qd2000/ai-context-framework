...
`v0.0.3.86` 已完成正式 release/global install；当前 priority-100 主线是 continuation liveness issue `d3456d0b5cf5c261652d`：解决真实长时间 physical execution 仍在运行时 heartbeat 跨 stale window 导致 duplicate scheduler wake 误 recover 的通用根因。该 P0 必须先完成 deterministic regression、docs/Automation、release 与 installed-state proof。
