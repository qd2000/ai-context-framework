...
`v0.0.3.84` 已完成历史 merge/tag identity，但 exact trusted-publishing workflow `32984611066` 已终态 `completed/cancelled`，PyPI `0.0.3.84` 未发布；该 external effect 只允许 reconcile 为 failed/cancelled，禁止重放 `.84`。当前 durable authority 已升级为 P0 control-plane reliability + Observer V2：先收口 `ACF_CONTINUATION_FENCE_TOKEN_FILE` credential handle、canonical Windows `acf.cmd` 与 closeout authorization resolver，再按 P1 Target Registry/run history → P2 target narrative/Map Review/problem model → P3 Chinese human-first Dashboard V2 → P4 ACF/FCC/AStockT_AI real dogfood 推进。最终 Production acceptance 只从包含这些变化的最新稳定 global install 起算；WS012 始终保持 long-lived Maintenance，不得标记 Done/Cancelled/Archived。
...
`.84` publishing 已 terminal-cancelled 且 PyPI absent，不再是可安装 baseline。当前 candidate scope 为独立 P0 reliability checkpoints（fenced credential transport、Windows `acf.cmd`、closeout authorization）以及其后的 Observer V2 P1–P4；大型 Observer 重构不得与 credential reliability checkpoint 混成单一 diff。
...
v0.0.3.84 原 candidate：108 continuation CLI tests passed；WS012 file-scoped guard / git diff --check / full release_check v0.0.3.84 均通过。当前 Unreleased credential transport focused regression 已覆盖 claim/assert-owner/heartbeat/workspace-intent/checkpoint/release 与 challenge-backed recover file transport；完整候选 gate 仍需在本修复 checkpoint 前继续执行。
