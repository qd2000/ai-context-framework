...
`.88` release / global install / continuation closeout 已完成并禁止 replay。当前详细产品 authority 已切换到 `OBSERVER_HTML_VISUALIZATION_PLAN.md` 的 **Observer Agent-first v2.0**，`PLAN.md` 只保留当前路线与指针。priority-100 durable directive `dir-153b33e6d0cb4bccbb67` supersede 旧 `dir-41b8581a296544399208` v1.2 fixed-renderer 后续路线：Project Observer 的主体是自主 Agent，ACF 降为可选导航/数据/维护工具和参考材料，不再作为唯一事实来源、页面生成前置或固定 renderer。

当前阶段已从旧 H1 renderer 扩展路线切换到 **S1→S2**：S0 authority/directive 当前态一致性已经落盘，generation 166 已直接用真实 WS012/ACF 资料生成首份 Agent-authored candidate 页面 `output/observer/candidate/index.html` 和简明接续记录 `output/observer/OBSERVER_NOTES.md`。该生成链没有依赖 `primary_visualization.kind/spec`、Map Review/presentation lifecycle、`acf observer snapshot` 或 ACF deterministic renderer；旧关系图、expected-target、run/history/archive/concurrency 等已验证成果只作为兼容数据/参考，不作为新页面必须经过的状态机。静态自包含检查、file-scoped guard 与 strict docs check 已通过，但真实浏览器视觉/交互和用户人工 PASS 仍未取得。
...
当前 next action：用 S1 真实切片进入 S2 调用关系审查与产品减法。优先核对 `automation_contracts.py` 与 Observer presentation/render/storage 的真实调用者，把 explicit Registry display gate、每轮 semantic-review/Map Review、fixed renderer 等从新 Observer 的必经前置降为 optional/legacy，并确保旧 snapshot/maintenance 不覆盖 Agent-owned candidate/latest；同时保留 Writer continuation ownership/effect/fencing、真实 run/history 与 Production anti-masking。随后在合法访问时把 Agent-authored 路线覆盖到 FCC WS079/WS080/WS086。不得先设计新的通用 renderer/API/schema/plugin platform，也不得通过 Maintenance snapshot 刷新 canonical Production Dashboard。
