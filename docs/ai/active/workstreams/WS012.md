...
当前阶段从旧 H1 renderer 扩展路线切换为 **S0→S1**：先完成 authority/directive 当前态一致性，然后直接用真实项目资料生成首份 Agent-authored candidate 页面及稳定 candidate 入口；生成过程不得依赖 `primary_visualization.kind/spec`、Map Review/presentation lifecycle 或 ACF deterministic renderer。旧关系图、expected-target、run/history/archive/concurrency 等已验证成果可作为兼容数据/参考，不作为新页面必须经过的状态机。
...
当前 next action：完成 `dir-153b33e6d0cb4bccbb67` 的 evidence-backed adopt，并在当前合法可达的真实项目范围直接落第一份 Agent-authored candidate 页面；若固定 ACF worktree 无法合法写 FCC 输出，则先用 ACF/WS012 证明新路径，再在后续合法访问时覆盖 WS079/WS080/WS086。不得先设计新的通用 renderer/API/schema/plugin platform，也不得通过 Maintenance snapshot 刷新 canonical Production Dashboard。
