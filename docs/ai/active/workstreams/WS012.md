...
`v0.0.3.87` 已成为 immutable failed release identity：GitHub master/tag 已形成，但 release workflow 在 publish 前因跨平台 CI regressions 失败，PyPI 无 `.87`。当前 continuation/overconstraint 阶段必须先一起收敛 `.87` 暴露的跨平台 CI 问题、ownerless graceful-handoff provenance deadlock 与 tool-backed stale-owner wait/await，再进入一次新的 combined stable successor release/global install。ownerless deadlock 已在 generation 79 形成产品级修复并完成真实 WS012 dogfood：`workspace reconcile-handoff` receipt `26e17f8b-aa46-4731-a164-e38ce519933e` 后 `doctor.can_claim=true / blocked_reasons=[]`，随后正常 claim gen79；实现继续保持未知 dirty drift fail-closed，并要求 HEAD 漂移显式 exact `--accept-head`。
