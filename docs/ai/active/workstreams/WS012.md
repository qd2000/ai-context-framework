...
`v0.0.3.85` 已完成 immutable release / global install，WS012 已回到长期 Maintenance。2026-09-03 约 10:30 +08:00 暴露跨项目 user-level `ACF_HOME` state-loss incident：旧 continuation / Observer / authorization runtime 不可恢复，必须以 durable Git/PLAN/Runtime/PyPI/installed-state authority 重建 control baseline，且禁止重放 pre-recovery external effects。recovered continuation 只允许经 authenticated challenge-timeout + reconcile/recover 正式接管，不把 stale lease 当活 owner；Production Observer 已重新产生独立 post-loss activations，但 Target Registry 曾随 state loss 清空，已仅通过 user-level `target-set` 恢复 `ws012-writer` registration，后续 acceptance 只能从新的 Production activation 重新计。根删除来源尚未证明；同时已确认 `init/simplify --force` 现有实现缺少 `ACF_HOME` destructive-overlap guard，作为可复用 containment defect 修复并登记 issue `fe35056d9392b95206b7`，不得把该可复现风险冒充本次删除 root cause。
