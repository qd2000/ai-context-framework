...
- Review scope: 文件级；同步 Workstream guard 文档契约、旧项目迁移修复和 v0.0.3.52 稳定版版本事实。
...
3. 当前版本事实为 `v0.0.3.52`：`acf --version`、`pyproject.toml`、`uv.lock` 和本地包元数据应保持同步。
...
1. 当前阶段处在 P2 收尾和阶段 6「事实与注意力治理」深化之间；WS002 模块化拆分、WS003 Workstream-first 上下文治理、WS004 旧项目升级审计和 Workstream guard 文档/迁移修复均已完成，`v0.0.3.52` 是当前推荐正式使用稳定版本。
...
- 日期：2026-06-09
- 更新原因：同步 Workstream guard 文档契约、旧项目迁移修复和 `v0.0.3.52` 当前稳定版本事实。
