...
| T005 | Active | 分批 parser composition | T004 | 各命令模块 register_*_parser、runtime.build_parser 瘦身、命令树形状快照回归 | 前置抽取完成：add_json_argument/add_write_arguments 移到新叶子模块 ai_context_framework/cli_arguments.py，DEFAULT_STALE_DAYS 与 VALID_FEEDBACK_STATUSES/VALID_HUMAN_INDEX_STATUSES 移到 constants.py，并让 runtime_parts/doctor.py、knowledge_review.py、check.py 改为直接导入（消除注入依赖）。已下移 4 个批次共 13 个命令组的解析器构造：review/audit/curate/doctor（commands/review_audit_curate.py、commands/doctor.py）、linkify/link（edit_link.py）、decisions（decisions.py）、draft（next_status.py）、feedback（feedback.py）、human（human.py）、writeback（new_context.py）、task（plan_task.py）、archive（archive.py）。关键设计：命令组解析器必须注入 handler，因为 CLI 绑定的是 runtime_parts 的 deps 适配器而不是命令模块同名函数——这一语义漂移正是快照护栏抓到的。runtime.py 1988 -> 1816 行。证据：命令树快照 210 条路径零漂移、unittest 422 项（292 CLI + 130 杂项）+ 2 项快照/契约全绿、check template/strict、minimal_smoke 8/8。 | 继续剩余批次：knowledge/draft(已部分)/plan(new/edit workstream worktree 等大块尚未下移)；每批完成后跑 tests.test_cli_surface_snapshot 验证零漂移。 |
