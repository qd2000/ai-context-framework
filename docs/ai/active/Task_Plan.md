...
| T005 | Active | 分批 parser composition | T004 | 各命令模块 register_*_parser、runtime.build_parser 瘦身、命令树形状快照回归 | 继续下移 knowledge（6 子命令）与 edit（section/table 4 子命令）两组解析器到 commands/knowledge.py 与 commands/edit_link.py；VALID_KNOWLEDGE_STATUSES 移到 constants.py 并让 runtime_parts/check.py、knowledge_review.py 直接导入。累计下移 15 个命令组。runtime.py 1988 -> 1742 行。证据：命令树快照 210 条路径零漂移、unittest 416 项（292 CLI + 124 杂项）+ 快照/契约全绿、check template/strict、minimal_smoke 8/8。 | 剩余大块：plan(118 行)/new(108 行)/workstream(278 行)/worktree(130 行) 及 status/next/init/simplify/upgrade/check/log/links/version/observer/continuation 等；每批完成即跑快照护栏。 |
