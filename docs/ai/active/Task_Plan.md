...
| T005 | Pending | 发布并全局安装 v0.0.3.92 | T004 | 版本更新、CHANGELOG、merge/tag/publish/global install/installed-state evidence | version set 至 v0.0.3.92 且 version show 显示 cli/pkg_info/pyproject/uv_lock 一致；CHANGELOG 新增 .92 条目、README 写入 release candidate 口径、PKG-INFO 同步；release gate 仓库门禁通过（template、strict、minimal smoke、unittest 646、full upgrade matrix 28 fixtures）并在干净产物目录完成 wheel+sdist 隔离安装 smoke（acf v0.0.3.92）；.92 产物仅含 commands/observer.py 墓碑 | 等待人工批准后合并 master、创建 immutable v0.0.3.92 tag、push 触发 PyPI 发布，并执行 update_acf.ps1 全局安装与 installed-state 验证。 |
