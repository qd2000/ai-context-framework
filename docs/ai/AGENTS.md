...
6. `active/Workstreams.md`（只读取全局摘要；未显式选择 Workstream 时，不继续读取任何详情文件）
...
- 新会话默认只读取全局上下文和 `active/Workstreams.md` 摘要。存在一个或多个 Active / Blocked / ReadyToMerge / Merging Workstream 都是正常状态，不构成默认选择，也不得因此读取任何 WS detail、read_scope、reference、output 或专属 worklog。
