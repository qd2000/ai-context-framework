...
Generation 214 继续只读消费新的自然 Production 运行，没有由 Maintenance 刷新 canonical Observer runtime。FCC installed `.89` 在 07:35:08 +08 启动并于 07:35:27 +08 成功结束，形成 stable rev345，`errors=[]`、`worktrees_scanned=11`、lock idle、data fresh；Agent-owned `index.html` 于约 07:37 自然更新，strict UTF-8 回读继续为 U+FFFD=0。rev345 项目事实把 WS079 进一步收窄到 E216 terminal-positive：required boundaries + feed PHFlash + one bounded Column Run 仍成立，但 `CurrentIteration=0`、`CfsConverged=false`，read-only post-Run evidence 指向 Condenser/Reboiler duty readiness 与 Main TS not-solved；E217 仍只是 read-only WIP，不构成新的 solve/convergence crossing。此前 rev343 notes 中的 6 个字面量 `` `r`n `` 仍保留在历史文本，但本次 rev345 新增段落已恢复为正常换行格式，没有再次复现该写入模式，因此继续维持 prompt/project-specific Observe，不升级为 reusable ACF issue。ACF installed `.89` 随后在 07:44:44 +08 自然运行并于 07:44:51 +08 成功结束，形成 rev779，Dashboard success、`errors=[]`、`worktrees_scanned=4`、data fresh；primary Agent-owned page 于约 07:47 自然更新，strict UTF-8 / U+FFFD=0。当前固定 DevSpace Local 仍无法独立定位 AStockT_AI 项目根，因此没有取得比既有 g210 样本更新的可验证 stable-entry evidence，也不据此推断成功或回归。本轮没有发现新的 reusable ACF/Observer defect；`83063f341ef586d70017` 继续保持 open candidate-only，successor/release boundary 不变。

Generation 215 又取得新的自然 Production 样本，Maintenance 仍未执行 `snapshot / interpret / narrative / render`。FCC installed `.89` 在 08:39:24–08:39:42 +08 成功形成 stable rev348，`errors=[]`、`worktrees_scanned=11`；Agent-owned stable `index.html` 于约 08:40 自然更新并经独立 strict UTF-8 回读确认 U+FFFD=0。rev347/rev348 同时暴露了辅助 notes 完整性的重复现象：`observer_notes.md` 仍 strict UTF-8 / U+FFFD=0，但 08:36–08:39 新增段再次以字面量 `` `n `` 作为分隔符；全文件当前计数为 13 个 `` `n ``，另保留 rev343 历史的 6 个 `` `r`n ``。因此该现象已从“一次性”升级为 **FCC prompt/project-specific 可复现 formatting defect**，但 ACF primary notes 未出现对应新增格式损坏，FCC stable page 也未受影响，当前仍缺少跨项目或 ACF 存储/renderer 共因证据，故暂不登记新的 global continuation issue。rev348 还曾观察到 WS079 generation 186 刚越过 stale threshold 且 E217 effect active/unresolved；随后 fresh stable continuation doctor 已证明 generation 187 在 09:15 +08 重新取得 fresh owner，effect journal 已恢复为 `36 completed / 56 historical failed / 0 unresolved`，所以该 rev348 告警属于被后续自然 recovery 消解的瞬时协调状态，不冒充 continuation 产品缺陷。ACF installed `.89` 在 08:47:29–08:47:35 +08 自然形成 rev780，Dashboard success、`errors=[]`、`worktrees_scanned=4`、data fresh；primary Agent-owned page 于 08:49 +08 更新并 strict UTF-8 / U+FFFD=0。AStockT_AI 仍未从当前固定 DevSpace Local 的 `C:\PROJECT` / `D:\PROJECT` 可访问面定位项目根，因此没有新增可验证 stable-entry 样本。Fresh Git 仍为 WS012=`7b97641`、credential-hardening=`79ad2cd`、release integration=`cee21f9`，最高 immutable tag 仍为 `v0.0.3.89`；successor/release boundary 未改变，issue `83063f341ef586d70017` 继续等待统一 immutable successor + global install + installed-state natural proof。

Generation 216 的自然 Production 证据进一步扩展了同一 text-integrity root cause。FCC installed `.89` 在 09:34–09:35 +08 先经历 rev349 unstable，再 bounded resample 为 stable rev350，`errors=[]`、`worktrees_scanned=11`、audit 0/0/0；Agent-owned `index.html` 于约 09:44 自然更新。rev350 notes 明确记录：通过 DevSpace PTY 直接传输 Unicode 时可复现**不含 U+FFFD、仍能作为合法 UTF-8 解码的 mojibake**，随后通过 ASCII-safe Base64 transport + byte write 才恢复正确可见文本，并在修正一个 U+515E→U+5176 字符后得到最终 SHA256=`ccc215af44fe75af6902bedced51b4edc74326c6855cdc22f03177cf376e0044`。这证明 g208 的 strict UTF-8 + U+FFFD=0 contract 是必要但不充分条件；issue `83063f341ef586d70017` 因而仍是同一 reusable root cause，而不是另建重复 issue。candidate contract 应额外要求从当前 source/authority 派生 expected visible-text marker 做晋升前 readback，并在 Unicode 通道保真未被证明时使用 byte-preserving 或 ASCII-safe transport；失败继续沿用 last-good/update-failure。同期 ACF installed `.89` rev781 在 09:49–09:50 +08 success/fresh、`errors=[]`、`worktrees_scanned=4`，primary Agent-owned page/notes 自然更新且 strict UTF-8 / U+FFFD=0；其中反引号 newline pattern 仅出现在描述 FCC 历史缺陷的文字中，不是 ACF 自身格式损坏。FCC notes 的历史 escape-formatting 计数仍为 13 个 `` `n `` + 6 个 `` `r`n ``，rev350 新段使用正常换行，故该项目特异 defect 本轮未再次增长。Maintenance 仍未执行 `snapshot / interpret / narrative / render`；AStockT_AI 仍无法从当前固定 DevSpace Local 独立定位，因此不制造新样本或回归结论。
...
1. **Fenced credential transport（历史 P0 方案，已被 post-WS012 `.90` 安全合同取代）**：该阶段曾采用环境变量 token-file 作为过渡方案；`.90` 不再把该环境变量或 raw secret 参数作为公共认证入口。当前合同以 `owner_context.handle` 为唯一正常 owner-protected 公共 transport，必要的 pre-.90 active-owner 兼容只允许显式本地 token-file migration，并保持原 lease/generation/runner identity 与 fail-closed 语义。历史 P0 验证事实保留，但不得据此恢复旧 transport。
...
- Dashboard shortcut 唯一维护在 `%USERPROFILE%\Documents\ACF Project Observer.lnk`，只更新原快捷方式 target，不创建版本/时间戳副本；
...
该段记录的是当时 Immediate repair 的历史合同；post-WS012 `.90` 已进一步收紧公共边界：owner-protected parser 只接受 `--owner-file` 与业务参数，不再接受 raw credential、公开 lease/generation assertion，也不从旧 token-file 环境变量读取认证材料。若升级现场仍有 pre-.90 active lease 且合法 owner 保留正确本地 token file，只能通过显式 `continuation owner migrate-legacy-token-file --legacy-token-file <local-path>` 在进程内验证并迁移到同 lease/generation/runner 的 owner context；credential 不进入 argv/JSON/prompt/log，旧文件退休失败则回滚新 context。该更新不改变本段记录的历史 defect/Immediate sequencing，只替换后续应采用的产品 transport contract。
