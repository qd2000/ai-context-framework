...
`coordination attempt` 为每个到达 runner 记录 compact `attempt_id / started_at / objective_summary`；这里的 summary 只是当前 goal label，受字节上限约束，但不定义单次 execution 的工作量边界。没有 owner 时 attempt 只是 `claim_candidate`。有 fenced owner 时先看 lease liveness：`fresh` owner 返回 `live_owner_observed`，登记 contender observation 但**不自动 challenge**，并暴露 `duplicate_wake_safe_to_yield=true`；只有 `stale / legacy_unknown` owner 才自动 open/join 当前 owner generation 唯一的 nonterminal challenge。显式 `coordination challenge` 仍可在调用方有独立理由需要重新确认 owner 时使用；challenge 不授予写权限，也不证明 owner 已死亡。任何可定位到项目的普通 ACF 命令都可以在 stderr opportunistic 提示当前 pending challenge，但该 probe 只读、不更新 coordination state，也绝不能冒充 ACK。只有 owner-protected continuation 命令成功解析当前本地 `owner_context.handle`，并完成 lease identity、generation fencing、runner binding 与 credential-possession 校验后，才记录 authenticated owner activity；正常 release 单独把 challenge 解析为 `owner_released`。deadline 到期只得到 `timed_out / ownership_forfeiture_candidate`，后续仍需 formal reconcile workspace/HEAD/effect/identity；ACK/reconcile/recover 继续共享 continuation state lock 与 generation fencing，因此竞态最多产生一个 writer。

`claim` 在本地 OS 文件锁保护下取得单一 active lease；第二个执行器看到 active lease 时不能 claim 或写 worktree，但可以作为 contender 登记 attempt/challenge 并等待 authenticated response、owner release 或 formal recovery。新 claim 会把 control generation 单调递增，并在提交 fresh owner 前先创建 task-local `owner_context.handle`。公共结果只返回该 opaque handle 与 lease/generation 等非秘密 metadata；handle 指向的受限本地 capability 绑定 workspace、task、lease、generation、runner 与 reusable credential，canonical lease 只保存 credential 的 SHA-256 verifier。delivery 失败不提交 lease、不推进 generation；正常 release 尝试清理 capability，新 generation 或 lease 消失仍使未清理的旧 handle 确定性失权。旧 raw-secret 参数不再属于 parser schema，旧 token-file 环境变量也不再作为认证输入。

`assert-owner` 及其他 owner-protected 命令只从 `--owner-file <owner_context_handle>` 加载 capability；公共 parser 不再接收 `--lease-id`、`--generation` 或 `--fence-token`。lease id / generation / credential 从 owner context 内部取得，并与 active lease 的 identity、runner binding、generation fencing 和 verifier 一并校验。owner-protected continuation 命令在凭据验证成功后会执行 authenticated coordination touch，因此可以响应当前 generation 的 pending/timed-out challenge；handle 缺失、不可读、schema 非法、绑定错误、credential 不匹配、stale generation 和普通 ACF 命令都不能 ACK。新 generation 生效后，旧 owner 的 `assert-owner`、`heartbeat`、`renew`、`checkpoint` 和 `release` 都会被确定性拒绝。该 fencing 保护的是 ACF continuation 控制协议；它不能阻止绕过 ACF 直接写外部系统，因此 non-idempotent 外部动作仍应在动作前显式 assert-owner。
上述 owner 校验是 binding-first 且跨平台保持同一公共错误合同：handle 文件系统边界、schema、workspace/task/runner 绑定先于 active lease 的 lease identity、generation 与 credential possession 检查；已提供 handle 的 schema、绑定、lease identity 或 credential possession 失败均返回 `owner_context_binding_mismatch`，不暴露 `owner_context_invalid`、`lease_mismatch` 或 `fence_token_*` 深层错误码；generation fencing 的状态错误仍按独立 fencing 合同返回。只有 handle 缺失或不可用时，才保留 `owner_context_required` / `owner_context_unavailable`。

升级时若 active lease 已存在但没有当前 owner context，`doctor/prompt` 保持 `owner_context_migration_required`。合法 pre-.90 owner 若仍持有旧本地 token file，可显式运行 `continuation owner migrate-legacy-token-file --legacy-token-file <local-path>`；CLI 只接收本地路径，credential 在 ACF 进程内读取并按当前 lease verifier 校验，成功后生成同 lease/generation/runner 绑定的 owner context 并退休旧文件，不改变 active owner identity。旧文件无法安全删除时撤销新 context 并 fail-closed；没有正确 token file 时禁止根据 verifier 或公开 metadata 重建权限，只能等待旧执行链释放或进入正式 stale/expired reconcile/recover。
...
`recover --reconcile-id ... --runner-id ...` 只消费 recorded eligible receipt。执行前重新计算整个 observation；lease state/id/generation、liveness class、HEAD、state status、round/effect digest 或 workspace ownership digest 任一变化都会返回 `reconciliation_stale`，要求重新 reconcile。成功后旧 round（若为 fenced round）以 `reconciling/superseded_by_recovery` 收口，新 lease generation 单调递增，公共结果只返回新建本地 capability 的 `owner_context.handle`，不返回原始 credential；workspace manifest 同步转移到新 generation，保留原 write intents 与可证明的 `task_owned` WIP，同时继续保护不重叠的 baseline/external dirty；恢复动作不会 stash/reset/clean，也不会为了换轮次自动提交 task/external WIP。`last_recovery.json` 只记录 compact recovery identity/digest。因此旧 owner 即使随后复活，也无法 heartbeat/renew/checkpoint/release 新 generation。`reconcile/recover` 只确定性执行调用方已经明确提供的 evidence assertion，不自动裁决外部事实。
...
- fenced round 的原始 owner credential 只保存在受限本地 owner-context capability 中；公共结果只交付句柄，canonical lease 等普通持久态只保存 verifier，不保存原始 credential；
