...
generated prompt 使用 goal-directed continuous execution，而不是把 scheduler wake、bounded Gate 或某个局部里程碑当成 Agent 工作量限制：scheduler wake 只负责从持久状态恢复持续任务，不定义独立工作回合、汇报周期、工作配额或预期停止点；bounded 只约束 ownership、写入范围、外部副作用和恢复风险。当前 stage/next_action 是 resume context/hint，不是唯一允许完成的微任务或停止边界。Agent 在有效 ownership 内自主决定工作范围、执行顺序、实现策略和验证深度。generic protocol 不再使用 1→14 的顺序 checklist，而按 startup/ownership、contention/recovery、workspace writes、liveness、effects、progress/checkpoints、actual handoff 与 issue reporting 分成 when-applicable 规则组，并明确这些规则不是工作流程，读到最后一节也不产生 session-end 信号。checkpoint 只保存进度，Git commit 只形成自然语义检查点，Gate 完成只进入下一次判断；只要仍有安全、非重复且有价值的工作，就不得把这些事件当成 release 或退出理由。final assistant response 本身会结束当前 execution session，因此不能为了汇报进展、已经运行了一段时间、完成了若干步骤、上下文看起来很长或主观感觉“该收尾了”而触发。某个动作被 fail-closed 只禁止该动作，Agent 应继续安全诊断、证据核查、issue 记录、规划、测试或其他不冲突工作。
...
`continuation prompt --json` 稳定暴露 `identity`、`current`、`execution_policy`、`project_context` 和 `scheduler_wrapper_contract`。`execution_policy` 机器字段明确 scheduler wake 仅为恢复入口、protocol 不是顺序 checklist、next_action 不是 work quota、final response 为 session 终止动作、进度汇报和 elapsed time 不是 session-end reason、timing 不是 execution-duration target、platform boundary 需要 explicit signal、release 不是默认收尾步骤。`project_context` 包含 state 中的 `plan_refs`、`constraints`，并定义外层 wrapper 的项目专用约束槽；该槽用于 tooling、Runtime、resource/VM/node、permission、security、scientific、validation 和 issue-reporting 规则。不同项目可填入完全不同的约束，但不得把 generic continuation 状态机复制进 wrapper。
...
coordination attempt
    ...
    │      ├─ owner authenticated touch → owner_active → contender 只让行重复 writer session，mission 继续
