...
| Top-Level Implementation Gap | 实施差距矩阵 | [reference/Top_Level_Implementation_Gap.md](Top_Level_Implementation_Gap.md) | Useful | 高 | 将 ACF 顶层设计逐项映射到当前实现，标记已实现、部分实现、未实现、不应实现、需补测试、需补文档和需补 upgrade 兼容项。 | 进入新代码 PR 前按需读取，优先选择 gap-driven 实施切片；当前推荐下一步为用真实 dogfooding 反馈驱动小边界修正。 |
| Tool Gap Review 2026-06-08 | 调研与问题清单 | [reference/Tool_Gap_Review_2026-06-08.md](Tool_Gap_Review_2026-06-08.md) | Useful | 中 | 基于外部工具官方资料、第一性原理和本仓库 dogfooding 现状，梳理 ACF 当前安装、上下文选择、多 agent UX、跨工具生态、评测闭环和数据治理候选问题。 | 选择下一批产品切片或整理 Feedback/Task Plan 前按需读取；不要把其中候选项直接视为当前事实或已承诺计划。 |
| Generated Marker Sync Design | 设计文档 | [reference/Generated_Marker_Sync_Design.md](Generated_Marker_Sync_Design.md) | Useful | 高 | 定义 Knowledge、ADR 和 Archive sync 的 generated marker 契约、同步边界、删除策略和 JSON 契约。 | 修改 `knowledge sync`、`decisions sync`、`archive sync` 或通用 marker helper 前必须读取。 |
