...
2. 当前任务状态为 Active。
3. check / strict check 均失败，唯一结构错误是缺少 `reference/Context_Curation_Prompt.md`。
...
3. path 为 `active/Current_Task.md`。
...
2. `active/Current_Task.md`：Active 当前任务缺少 dated status note。
3. `active/Context.md`：缺少 `Last reviewed` / `上次审阅` marker。
