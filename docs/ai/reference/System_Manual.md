...
Agent-owned 文本稳定入口在晋升前必须能够 strict UTF-8 回读，且不得包含 Unicode replacement character（U+FFFD）；检查失败按更新失败处理并保留 last-good。这是直写页面的最小文本完整性保护，不重新引入统一 renderer/schema。
