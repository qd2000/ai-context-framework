...
6. release preparation 前先解决 immutable version identity collision：当前 candidate 仍报告 `0.0.3.84`，而 canonical Git `v0.0.3.84` 已绑定历史 merge commit，禁止 reuse/repoint/retag。必须按当时 Git/tag/PyPI/release authority 选取一个新的未使用 identity，再同步正常 version/package/docs metadata 并执行正式 release Gate。
