...
截至 2026-09-01 generation 93，P0 credential transport、canonical Windows `acf.cmd`、closeout authorization、Observer V2 P1/P1.5/P2/P3 均已有独立 checkpoint；P3 product/runtime Gate 已由 `0e2137f9af74a9dda38553814e168f17745ea5f3`、650 full-unit tests、93 P3 focused tests、8 package-skeleton tests 与 28 个 integrated presentation lifecycle tests 收口。directive revision 36 又 supersede 了此前“等待 sibling-task mutation authority”的 pre-P4 persisted next_action：四个 enabled Writer Scheduled Task 已在 authorized interactive context 原位迁移，本 generation 对 **当前 live task identity + full prompt** 做独立 read-back 后确认 WS079、WS080、WS086、AStockT_AI 均满足 §7.2 的 `keep / move-to-generated / preserve-extension` 目标，未发现需要继续保留的 family-specific prompt delta。因此旧的 vague waiting Gate 已满足/退休，当前 implementation stage 正式进入 **P4 real-project candidate dogfood**。这不等于 P1.5 installed-state acceptance，也不等于 WS012 long-lived mission 完成。当前默认执行顺序为：

1. 以当前 audited candidate commit 为 product-under-test，对 ACF/WS012、FCC WS079/WS080/WS086 与 AStockT_AI 做 P4 real-project dogfood；保持 candidate evidence 与 global stable installed-state evidence 分离，candidate 不改写 canonical continuation state。
2. 验证 Target Registry / target-local projection / Project Overview decision、真实 plan/strategy change、unexpected blocker、route-impacting issue、external/tooling transfer、abnormal run、map/presentation change、Windows CMD、fenced credential 与 anti-masking contracts；只记录真实可复现 evidence，不制造 synthetic route/map changes。
3. Maintenance 的 candidate validation 只能作为明确标识的 product-under-test evidence；独立 Production Observer activation 继续是 Production freshness/acceptance 的唯一来源，普通 Maintenance wake 不通过 snapshot/interpret/narrative refresh 人为养鲜。
4. release preparation 前先解决 immutable version identity collision：当前 candidate 仍报告 `0.0.3.84`，而 canonical Git `v0.0.3.84` 已绑定历史 merge commit，禁止 reuse/repoint/retag。必须按当时 Git/tag/PyPI/release authority 选取一个新的未使用 identity，再同步正常 version/package/docs metadata 并执行正式 release Gate。
5. 新 immutable stable release + global install 后再做 installed-state P1.5/P4/credential-transport dogfood，并仅从包含全部变化的最新 stable Production Observer activations 重新计算 acceptance；单次 release/pass/checkpoint 不结束 WS012 Maintenance mission。
