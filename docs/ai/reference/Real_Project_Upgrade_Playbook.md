...
| 原有内容 | ACF 目标位置 |
|---|---|
| 当前项目事实、阶段目标 | `active/Context.md` |
| 当前任务 | `active/Current_Task.md` |
| 多步骤计划 | `active/Task_Plan.md` |
| 长期背景 | `reference/Project_Brief.md` |
| 架构说明 | `reference/Architecture.md` |
| 技术环境 | `reference/Tech_Context.md` |
| 重要决策 | `decisions/ADR-xxxx dot md` |
| 历史过程 | `worklog/` |
| 临时想法或需求 | `active/Feedback_Inbox.md` |
...
1. 是否补齐 `active/Feedback_Inbox.md`。
2. 是否补齐 `active/Task_Plan.md`。
...
启用后才会创建 `active/Workstreams.md`、`active/workstreams/` 和 `archive/workstreams/`，也只有这时 `acf check` 才执行 Workstream 一致性检查。
