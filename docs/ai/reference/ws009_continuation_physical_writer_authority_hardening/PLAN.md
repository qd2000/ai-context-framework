...
| WS009.5 | Active | Full regression, release and adoption | version bump、release_check、package/PyPI、global install、self/FCC/AStock smoke、merge/archive | `.65` Windows full release gate 全绿，但 GitHub run `32256066761` 在 Ubuntu 因两条 CRLF fixture 假设失败且未 publish；Lenovo stable `.65` 已验证 workspace v3；`.66` fixture 已改为显式 LF bytes + 强制 Git checkout 重建；prepared/no-external-id no-start recovery 已由 `49d6e50` + `c53df87` 收口；PR #1 run `32272791254` 已证明 Ubuntu 3.10/3.12 + Ubuntu/Windows package smoke 全绿，同时在 Windows 3.10/3.12 暴露 TEMP 8.3 alias/canonical path 等价性缺陷 `4609d6ad7c724d0d6627`，最小 canonical-path 修复 focused 10/10；待 fresh local full + PR rerun 全绿后 tag/PyPI/installed-state adoption smoke |
...
继续 WS009.5：`.65` tag 已由 GitHub run `32256066761` 证明在 Ubuntu full unittest 的两个 CRLF fixture 断言上失败，未进入 PyPI；其本地 full-gated wheel 已在用户临时 self-hosting override 下安装到 Lenovo，并证明 stable `acf continuation doctor/prompt` 能直接读取 WS009 canonical workspace v3，FCC WS079/080/086 legacy v2 state 也可稳定只读。`.66` portability fixture 现在从显式 LF bytes 开始，并通过删除后 Git checkout 重建来验证 `eol=crlf`。generation 25 随后真实暴露 prepared/no-external-id effect 无法 ownerless 收口的缺口；该同一根因被并发轮次重复登记为 `8cbc888d5aee29afe337`、`f2885da871d8c2db00e9`、`e83bb6b4961abf1b5925`，现均已标记 resolved。用户授权的最小 emergency override 先以 GitHub authority 证明 `ws009-g25-pr-ci-066` 从未 submit，在不伪造 external id 的前提下将其收口为 failed，并通过 receipt `f8a35521-c0ff-43e8-96e9-80441f3455ab` 完成 formal generation 25 -> 26 recovery；产品侧 `49d6e50` 随后新增 `reconcile --effect-not-started`，只允许 `prepared + external_id=null + failed` 且要求 owner-ended/forfeiture 与外部 authority evidence，`c53df87` 进一步冻结 active effect 仍必须 fail-closed。随后 `.66` 本地 full release gate 通过并创建 PR #1；GitHub run `32272791254` 的 Ubuntu Python 3.10/3.12 与 Ubuntu/Windows package smoke 全绿，但 Windows Python 3.10/3.12 full unittest 同时因 GitHub runner TEMP 使用 `RUNNER~1`、`Path.resolve()` 返回 `runneradmin` 而出现 26 failures + 1 error，根因 fingerprint `4609d6ad7c724d0d6627` 已登记。修复不改变 JSON canonical-path contract：`check_context` 入口 canonicalize root，通用 containment helper 对 child/root 双方 resolve，relative-display 在 lexical relative 失败后使用 canonical fallback；7 条 changed_files 与 1 条 worktree target 测试改为 canonical-path 比较，代表性 focused 10/10。下一步形成 scoped checkpoint，push PR 新 head，并行运行 fresh `.66` local full release gate 与 PR cross-platform rerun；两者全绿后重新 collision probe，只创建一次 `.66` tag，让 GitHub release workflow 作为发布权威。公开发布后升级 Lenovo 与 qdspc 稳定安装态并完成 ACF-self、FCC continuation 与 AStock continuation smoke，最后进入 merge/archive 与 primary-maintenance authority cleanup。
