...
| WS009.5 | Active | Full regression, release and adoption | version bump、release_check、package/PyPI、global install、self/FCC/AStock smoke、merge/archive | `.65` Windows full release gate 全绿，但 GitHub run `32256066761` 在 Ubuntu 因两条 CRLF fixture 假设失败且未 publish；Lenovo stable `.65` 已验证 workspace v3；`.66` fixture 已改为显式 LF bytes + 强制 Git checkout 重建；本轮 self-dogfood 新增 `--effect-not-started` 安全分支，focused 5/5、continuation suite 68/68、template/strict/guard/diff-check 全绿；待 fresh full release/PyPI/AStock smoke |
...
继续 WS009.5：`.65` tag 已由 GitHub run `32256066761` 证明在 Ubuntu full unittest 的两个 CRLF fixture 断言上失败，未进入 PyPI；其本地 full-gated wheel 已在用户临时 self-hosting override 下安装到 Lenovo，并证明 stable `acf continuation doctor/prompt` 能直接读取 WS009 canonical workspace v3，FCC WS079/080/086 legacy v2 state 也可稳定只读。`.66` portability fixture 现在从显式 LF bytes 开始，并通过删除后 Git checkout 重建来验证 `eol=crlf`；本轮 generation 25 又真实暴露 prepared/no-external-id effect 无法 ownerless 收口的缺口，fingerprint `e83bce2213ba612d7975`，因此新增 `reconcile --effect-not-started`，只允许 `prepared + external_id=null + failed` 且要求 owner-ended/forfeiture 与外部 authority evidence。focused 5/5、完整 continuation 68/68、template/strict/guard/diff-check 已全绿。先形成 checkpoint，再用该 receipt 路径恢复当前 self-hosted round；随后运行 fresh `.66` full release gate。full gate 通过后重新 collision probe，只创建一次 `.66` tag，让 GitHub Ubuntu release workflow 作为跨平台发布权威：成功以 PyPI `.66` 可见为终态，失败以 GitHub failure evidence 修复前移到下一 immutable version。公开发布后升级 Lenovo 与 qdspc 稳定安装态并完成 AStock continuation smoke，最后进入 merge/archive 与 primary-maintenance authority cleanup。
