...
| WS009.5 | Done | Full regression, release and adoption | version bump、release_check、package/PyPI、global install、self/FCC/AStock smoke、merge/archive | 最终 code HEAD `6c58e24`：reclassification + effect-capacity fixes；continuation 73/73、full unittest 494/494、template/strict/guard/diff-check 全绿；exact-head Windows `release_check --mode full` 覆盖 full upgrade matrix、wheel/sdist build 与隔离安装全部成功；immutable `v0.0.3.66` tag 的 Ubuntu release workflow 通过后 PyPI 已发布 0.0.3.66；Lenovo `.65 -> .66`、qdspc `.63 -> .66`，ACF-self/FCC/AStock installed-state smoke 均通过。 |

2026-08-20 generation 29 self-hosting 新增 dogfood evidence：canonical `rounds.json` 已达到 16 条上限，其中 15 条历史 round 均为 formal recovery 写入的 `phase=reconciling`、`milestone=superseded_by_recovery` 且已有 `ended_at`，但 `begin_round()` 只裁剪 `released`，导致新的 stable `claim` 返回 `round_journal_full`。fingerprint=`bf2738af2d8bd9a4d977`。按用户长期 emergency override 授权先保留 canonical journal 完整备份，再只删除最老且已 `ended_at` 的 generation 9 recovery-superseded 历史项，使 stable control-plane 成功 claim generation 29；产品修复把这类已结束 reconciling 历史纳入 bounded journal 的安全裁剪条件，仍不裁剪没有 `ended_at` 的活动 reconciling round。新增容量回归先 RED 后 GREEN，完整 continuation suite 70/70、full unittest 491/491、template/strict、file-scoped guard 与 diff-check 均通过。

2026-08-20 generation 30/31 release dogfood 再现 workspace provenance 缺口 `fe272e56d2d04bb21e23`：release writer 权威结束后生成的 `ai_context_framework.egg-info/PKG-INFO` 属于合法 WS009 release output，但启动前漏报该具体 path 时会先落入 `unexpected_nonoverlap`，既不能静默 claim，也不能用事后的普通 `workspace intent` 越过 provenance guard。候选实现新增 fenced-owner `workspace reclassify`，只允许把**当前 unexpected path** 在 durable evidence + reason 审阅后显式归入 `task_owned` 或 `baseline_external`；task-owned 同时补 write intent 并继续受 bound Workstream direct write scope 约束，conflict、非 unexpected path、重叠、scope 越界和证据缺失继续 fail-closed。generation 30 随后 stale，matching challenge `78576728-0007-4c63-9d41-3ad73200258c` timed out；stable `.65` reconcile 已生成 eligible receipt `58f3fec7-7486-4224-9a34-5858fed385db`，但 stable recover 仍被已在 `a068b70` 修复的 `round_journal_full` 阻塞。按长期 emergency override 先备份 canonical state，再用已验证候选 control-plane formal recover 到 generation 31（recovery `f4601d78-40af-480d-8ec4-4e3e3446dc47`），随后 stable owner authentication 恢复正常。reclassification focused 2/2、continuation suite 72/72、full unittest 493/493、template check 与 strict check 均通过；README、Automation、template/dogfooding System Manual 与 CHANGELOG 已同步新 CLI 边界。

同一 release Gate 在 checkpoint `a28a414` 推送后收到新的真实 WS080 dogfood blocker `64b8c0b361ef5dcbfc1b`：其 canonical effect journal 已有 64 条记录且 64/64 全部 terminal、`unresolved=[]`，stable `effect prepare` 仍因 `MAX_EFFECTS=64` 直接 `effect_journal_full`。terminal effect 不能简单删除，否则旧 logical key 会失去 durable replay protection。WS009 因而只做 bounded capacity hardening：保留所有 terminal identities，把 `MAX_EFFECTS` 从 64 提升到 256，并把 effect-journal 独立字节上限提升到 256 KiB；round journal 仍维持原 64 KiB 上限。characterization 在旧实现上确定性 RED 于第 65 条，候选实现 GREEN；对 WS080 真实 57,890-byte / 64-record canonical journal 的只读内存 probe 可创建 hypothetical 第 65 条且 canonical bytes 完全未改写。

2026-08-20 release/adoption closeout：effect-capacity focused regression、continuation suite 73/73 与 full unittest 494/494 全绿；`acf check template`、`acf check --strict`、WS009 file-scoped guard 与 `git diff --check` 均通过。最终 release code checkpoint 为 `6c58e243c69994f50db433902fb71f873161b17b`。本地/远端/PyPI collision probe 均确认 `.66` 未占用后，只创建并推送一次 annotated `v0.0.3.66` tag；tag workflow 在 Ubuntu 再次执行 full release Gate 后 PyPI 出现 `0.0.3.66`，因此公开发布已权威完成。Lenovo stable 从 `.65` 升级为 `.66` 后可直接继续读取 WS009 generation 31 canonical state，workspace semantic-clean 仅保留三个 stat-only egg-info observations；FCC WS079/WS080/WS086-hourly continuation inventory/schema 仍兼容。qdspc/AStock stable 从 `.63` 升级为 `.66` 后 global status 仍 healthy，新 `continuation list --all-projects` 可发现 18 个既有 continuation task，当前 WS018 doctor healthy 并正确把 legacy workspace v2 报告为 explicit migration candidate。完成标准 1-11 的 release/adoption 部分至此满足。

两个历史 medium fingerprint `ab1ecb7d7e07ed277a84` 与 `44cf958c94dd26e14ab2` 仍描述“已审阅 baseline_external 在后续 scope expansion 时缺少显式 promotion”的便利性/lifecycle gap；它们不影响 logical single-writer、effect replay safety、recovery correctness、semantic-clean、schema upgrade 或 `.66` adoption。为避免在已通过 release Gate 后无限扩展 WS009，不在本 Workstream 继续改变 workspace schema/ownership contract；保留为后续候选，由新的 Workstream 在有新的真实 dogfood 需求时处理。
...
WS009 产品/release/adoption Gate 已完成。下一步仅做生命周期收口：形成 post-release closeout checkpoint，提交 merge request 并将 Workstream 置为 ReadyToMerge；使用 ACF worktree integration/promotion 路径合并到 primary；随后在 primary maintenance 路径归档旧 terminal Current_Task/Task_Plan、刷新 Context review/volatile version authority，最后把 WS009 标记 Done 并 archive。不要在这个阶段继续扩展 continuation 产品协议或重发 `.66`。
