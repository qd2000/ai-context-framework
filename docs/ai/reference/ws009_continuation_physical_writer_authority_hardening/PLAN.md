...
继续 WS009.5：核心 continuation/worktree/attention/schema hardening 已完成，当前只收口 `.66` release candidate。先对 generation 31 的 `workspace reclassify` + release metadata 形成 scoped checkpoint 并 push；随后以**该新 exact HEAD** 同时执行 fresh local `scripts/release_check.py --mode full` 与 PR Ubuntu/Windows × Python 3.10/3.12 + package/minimal-smoke CI。只有 dual-green evidence 才 resolve `fe272e56d2d04bb21e23` 与仍 open 的 release-portability/recovery fingerprints；然后重新 collision probe 本地 tag、remote tag 与 PyPI，只创建/推送一次 immutable `v0.0.3.66`，由 GitHub release workflow 完成 publish。公开发布后升级 Lenovo 与 qdspc stable ACF，完成 ACF-self、FCC continuation、AStock continuation installed-state smoke，最后进入 merge/archive 与 primary-maintenance authority cleanup；不再为发布便利性扩展 WS009 mission。
