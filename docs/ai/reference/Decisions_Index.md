...
| ADR-0004 | 将 acf 演进为可安装的 AI-facing 上下文维护 CLI | Active | acf 的长期方向是成为可在任意目录调用、主要面向 AI 使用的上下文维护 CLI，而不是通用 Markdown 编辑器。 | `decisions/ADR-0004.md` |
...
| ID | 标题 | 状态 | 摘要 | 需要确认的问题 |
|---|---|---|---|---|
| ADR-0005 | 使用可选 Workstream 层管理并行目标线 | Active | 引入可选 Workstream 层表达多 agent 并行目标线，但不做 runtime、调度器或权限系统。 | 详情：`decisions/ADR-0005.md` |
