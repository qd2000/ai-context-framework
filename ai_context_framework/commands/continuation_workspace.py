...
8. Inspect workspace ownership with `acf continuation workspace status ... --json`. Before modifying project files, declare the concrete paths with `acf continuation workspace intent ... --lease-id <lease_id> --generation <generation> --fence-token <fence_token> --path <path> ... --json`. A bound Workstream intent must remain inside its direct write scope. Existing baseline/external dirty is protected; do not stash, reset, clean, stage, or commit it. Narrow ACF control-plane lifecycle exception: when the current authenticated runner has just executed deterministic `acf workstream scope-add|merge-request|ready|merge-start|done` operations for the bound Workstream, the exact generated `docs/ai/active/Workstreams.md` and bound `docs/ai/active/workstreams/<ID>.md` diffs may be reviewed and committed in a dedicated control-plane checkpoint at the lifecycle/merge boundary. This exception never grants ordinary Task authority-file write scope, must not include any other baseline/external path, and requires durable evidence identifying the ACF command/output provenance. If a durable writer finishes with a reviewed in-scope output that was not declared before launch and is therefore `unexpected_nonoverlap`, do not silently claim it: the fenced owner may use `workspace reclassify --task-owned <path> --evidence-ref <durable-ref> --reason <review>` only after proving provenance; uncertain output remains external/fail-closed. After writes and before handoff/finalization, refresh ownership with `acf continuation workspace refresh ...` so task-owned, unrelated external, and true path conflicts are explicit.
