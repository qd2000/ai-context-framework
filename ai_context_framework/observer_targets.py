...
def _round_projection(target_id: str, row: dict[str, object]) -> dict[str, object]:
...
def continuation_round_history(target: dict[str, object], continuations: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for continuation in continuations:
        ...
        for raw in payload.get("rounds") or [] if isinstance(payload, dict) else []:
            if not isinstance(raw, dict):
                continue
            run = _round_projection(str(target["target_id"]), raw)
        ...
        rows.append(
            {
                ...
                "route_ref": (
                    finish.get("route_ref") if isinstance(finish, dict) else start.get("route_ref")
                ),
                "evidence_refs": list(
                    dict.fromkeys(
                        [
