...
def _target_view_html(
    ...
    *,
    ...
    return f"""
  <section class="target-panel {'is-active' if active else ''}" data-target-panel="{escape(target_id, quote=True)}">
    ...
    <section class="target-subsection"><h3>Run chain / 自动任务运行历史</h3>{_target_run_chain_html(view.get('runs'))}</section>
    <section class="target-subsection"><h3>Target Alerts</h3>{alerts_html}</section>
