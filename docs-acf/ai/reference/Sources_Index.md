...
| Top-Level Implementation Gap | 实施差距矩阵 | [reference/Top_Level_Implementation_Gap.md](Top_Level_Implementation_Gap.md) | Useful | 高 | 将 ACF 顶层设计逐项映射到当前实现，标记已实现、部分实现、未实现、不应实现、需补测试、需补文档和需补 upgrade 兼容项。 | 进入新代码 PR 前按需读取，优先选择 gap-driven 实施切片；当前推荐下一 PR 为 JSON contract consistency tests。 |
